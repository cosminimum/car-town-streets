        ...
        if (updateRecordsAsSynced) {
            for (ReportUsageData appUsage : usageList) {
                try {
                    ...
                    Logger.v(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "Usage: InstallStateReporter: handleResults() Updated record as synced [id:%1$d]", new Object[]{Long.valueOf(record.getId())}));
                } catch (Exception e3) {
    ...
    public void reportUsageInChunks(int batchSize, SyncableDatabase<?> database, List<ReportUsageData> usageList, HashMap<ReportUsageData, ? extends DatabaseRecordBase> usageToDBObject) {
        ...
        for (int i = 0; i < usageList.size(); i++) {
            ...
            if (currentUsageList.size() >= batchSize) {
                Logger.d(Area.USAGE.value(), String.format(Locale.US, "Usage: UsageReporter: reportUsageInChunks() Calling reportApplicationUsage for %1$d records", new Object[]{Integer.valueOf(currentUsageList.size())}));
