...
public class PackageMonitor extends BroadcastReceiver {
    ...
    public void doOnReceive(Context context, Intent intent) {
        ...
        try {
                ...
                CommContext commContext = CommManager.createContext(applicationKey, context, (ResultReceiver) new ResultReceiver((Handler) null) {
                    ...
                    public void onReceiveResult(int resultCode, Bundle resultData) {
                        for (String key : resultData.keySet()) {
                            Logger.d(Area.OS_ENTRY_POINT.value(), String.format(Locale.US, "PackageMonitor: Callback from the GetJar SDK [%1$s]", new Object[]{resultData.get(key).getClass().getName()}));
                        }
                    }
                ...
                try {
                    ...
                    try {
                        ...
                        if (bundle != null && !StringUtility.isNullOrEmpty(bundle.getString(OPERATION_KEY_VOUCHER_REDEMPTION_CHECK))) {
                            ...
                            Logger.d(Area.REDEEM.value() | Area.OFFER.value(), String.format(Locale.US, "PackageMonitor: doOnReceive(): voucher redemption [token:'%1$s']", new Object[]{voucherToken}));
                            if (!StringUtility.isNullOrEmpty(voucherToken)) {
                                ...
                                if (!isRedeemed) {
                                    ...
                                    Logger.d(Area.REDEEM.value() | Area.OFFER.value(), String.format(Locale.US, "PackageMonitor: doOnReceive(): voucher not redeemed, notification sent [token:'%1$s']", new Object[]{voucherToken}));
                                } else {
                                    Logger.d(Area.REDEEM.value() | Area.OFFER.value(), String.format(Locale.US, "PackageMonitor: doOnReceive(): voucher in redeemed state, no work to do [token:'%1$s']", new Object[]{voucherToken}));
                                }
                            }
                            ...
                            if (!intent.getBooleanExtra("android.intent.extra.REPLACING", false) && action != null && ("android.intent.action.PACKAGE_ADDED".equals(action) || "android.intent.action.PACKAGE_REMOVED".equals(action))) {
                                ...
                                Logger.d(Area.EARN.value(), String.format(Locale.US, "PackageMonitor: doOnReceive(): [packageName:%1$s action:%2$s]", new Object[]{packageName, action}));
                    ...
                    Logger.e(Area.OS_ENTRY_POINT.value(), String.format(Locale.US, "PackageMonitor: doOnReceive() %1$s", new Object[]{e2.getMessage()}));
                    Logger.d(Area.OS_ENTRY_POINT.value() | Area.EARN.value() | Area.USAGE.value(), "PackageMonitor: doOnReceive(): FINISHED");
                }
            }
        } catch (Exception e3) {
            ...
            Logger.d(Area.OS_ENTRY_POINT.value() | Area.EARN.value() | Area.USAGE.value(), str);
        }
    }
    ...
    private void checkForAndHandleManagedInstalls(Context context, String packageName) {
        ...
        if (context == null) {
            ...
            try {
                ...
                if (appState != null) {
                    Logger.d(Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "PackageMonitor: checkForAndHandleManagedInstalls() Host app %1$s is managing an EARN related event for %2$s", new Object[]{context.getPackageName(), packageName}));
                ...
                if (installReason != null && GooglePlayLaunchReason.REDEEM.equals(installReason)) {
                    Logger.d(Area.REDEEM.value() | Area.OFFER.value(), String.format(Locale.US, "PackageMonitor: checkForAndHandleManagedInstalls() Host app %1$s is managing a REDEEM related event for %2$s", new Object[]{context.getPackageName(), packageName}));
                    NotificationsUtility.showRedeemReminderNotification(context);
                ...
                Logger.d(Area.OS_ENTRY_POINT.value(), str);
            }
        }
    }
    ...
    private boolean shouldRetryTransactions(Context context, CommContext commContext) {
        ...
        try {
            ...
            Logger.d(Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "PackageMonitor: shouldRetryTransactions: [lastRetryTime: %1$d] [currentTime: %2$d] [delta: %3$d] [minimumDelta: %4$d]", new Object[]{Long.valueOf(lastRetryTime), Long.valueOf(currentTime), Long.valueOf(delta), Long.valueOf(minimumDelta)}));
