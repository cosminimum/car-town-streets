...
class InstallStateDatabase extends SyncableDatabase<InstallStateRecord> {
    ...
    protected static synchronized void initialize(Context context) {
        synchronized (InstallStateDatabase.class) {
            if (context == null) {
                ...
                Logger.i(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "InstallStateDatabase: waitForUserAccess() START [%1$s]", new Object[]{Logger.getShortStack()}));
    ...
    private InstallStateDatabase(Context context, String name) {
        ...
        Logger.i(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "InstallStateDatabase: Opened user specific database '%1$s%2$d'", new Object[]{DATABASE_NAME_PREFIX, Integer.valueOf(AuthManager.getInstance().getUserAccessId().hashCode())}));
    }
    ...
    public void addRecord(String packageName) {
        synchronized (this._databaseAccessLock) {
            if (StringUtility.isNullOrEmpty(packageName)) {
                ...
                Logger.v(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "InstallStateDatabase: Preexisting record found for '%1$s'", new Object[]{packageName}));
            } else {
                ...
                Logger.v(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "InstallStateDatabase: Added a FOUND_INSTALLED record for '%1$s'", new Object[]{packageName}));
                trimLruEntries();
            }
        }
    }
    ...
    public void updateState(long id, InstallStateManager.InstallState installState) {
        ...
        synchronized (this._databaseAccessLock) {
            if (checkForRecord(id)) {
                ...
                Logger.w(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "InstallStateDatabase: updateState() failed to find record %1$d", new Object[]{Long.valueOf(id)}));
            }
        }
    }
