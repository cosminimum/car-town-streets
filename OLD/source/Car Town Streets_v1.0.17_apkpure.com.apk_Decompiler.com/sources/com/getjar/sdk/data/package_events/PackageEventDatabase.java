...
class PackageEventDatabase extends SyncableDatabase<PackageEventRecord> {
    ...
    protected static synchronized void initialize(Context context) {
        synchronized (PackageEventDatabase.class) {
            if (context == null) {
                ...
                Logger.i(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "PackageEventDatabase: waitForUserAccess() START [%1$s]", new Object[]{Logger.getShortStack()}));
    ...
    private PackageEventDatabase(Context context, String name) {
        ...
        Logger.i(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "PackageEventDatabase: Opened user specific database '%1$s%2$d'", new Object[]{DATABASE_NAME_PREFIX, Integer.valueOf(AuthManager.getInstance().getUserAccessId().hashCode())}));
    }
    ...
    public void addRecord(String packageName, PackageEventManager.EventType eventType) {
        synchronized (this._databaseAccessLock) {
            if (StringUtility.isNullOrEmpty(packageName)) {
                ...
                Logger.v(Area.USAGE.value() | Area.STORAGE.value(), String.format(Locale.US, "PackageEventDatabase: Added a '%1$s' record for '%2$s'", new Object[]{eventType.name(), packageName}));
            }
        }
        trimLruEntries();
    }
