    ...
    private static void dumpSessionRatesToLogCat() {
        try {
            ...
            for (String name : _SessionRequestCounts.keySet()) {
                ...
                Logger.i(Area.USAGE.value(), String.format(Locale.US, "STATS: %1$s has been called %2$d times in %3$f seconds (%4$f requests per second)", new Object[]{name, Integer.valueOf(value), Float.valueOf(deltaSeconds), Float.valueOf(((float) value) / deltaSeconds)}));
            }
        } catch (Exception e) {
