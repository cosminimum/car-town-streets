...
public class DBTransactions extends SQLiteOpenHelper {
    ...
    private DBTransactions(Context context) {
        ...
        Logger.i(Area.TRANSACTION.value() | Area.STORAGE.value(), String.format(Locale.US, "DBTransactions: Opened user specific database '%1$s%2$d'", new Object[]{DATABASE_NAME_PREFIX, Integer.valueOf(AuthManager.getInstance().getUserAccessId().hashCode())}));
    }
    ...
    public static synchronized DBTransactions getInstance(Context context) {
        ...
        synchronized (DBTransactions.class) {
            ...
            Logger.i(Area.TRANSACTION.value() | Area.STORAGE.value(), String.format(Locale.US, "DBTransactions: waitForUserAccess() START [%1$s]", new Object[]{Logger.getShortStack()}));
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Logger.d(Area.TRANSACTION.value() | Area.STORAGE.value(), String.format(Locale.US, "Upgrading database from version %1$d to %2$d, which will destroy all old data", new Object[]{Integer.valueOf(oldVersion), Integer.valueOf(newVersion)}));
    ...
    public boolean checkForRecord(java.lang.String r12) {
        ...
        L_0x0041:
            ...
            com.getjar.sdk.logging.Logger.e(r5, r3, r1)     // Catch:{ all -> 0x0055 }
            goto L_0x003d
        L_0x0055:
            r2 = move-exception
            monitor-exit(r4)     // Catch:{ all -> 0x0055 }
        ...
        if (transaction == null) {
            ...
            if (!NotificationState.NONE.equals(oldState) && NotificationState.NONE.equals(newState)) {
                ...
                if (wasUpdated) {
                    ...
                    Logger.v(Area.TRANSACTION.value() | Area.STORAGE.value() | Area.EARN.value(), String.format(Locale.US, "DBTransactions: updateEarnTransactionNotificationState() Updated from %1$s to %2$s", new Object[]{oldState, newState}));
                }
                return wasUpdated;
            } else {
    ...
    private boolean updateTransaction(TransactionBucket transaction, String state) {
        ...
        if (transaction == null) {
            ...
            Logger.v(Area.STORAGE.value() | Area.TRANSACTION.value(), String.format(Locale.US, "DBTransactions updateTransaction [clientTransactionId:%1$s] [newState:%2$s]", new Object[]{transaction.getClientTransactionId(), state}));
            long currentTime = System.currentTimeMillis();
            ContentValues values = new ContentValues();
            values.put("timestampLastUpdated", Long.valueOf(currentTime));
