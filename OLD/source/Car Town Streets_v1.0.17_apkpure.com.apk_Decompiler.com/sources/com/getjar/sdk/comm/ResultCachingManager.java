    ...
    public Result getRequestResult(Operation operation) {
        try {
            ...
            if (cacheEntry != null && !cacheEntry.hasTtlExpired()) {
                Logger.v(Area.COMM.value() | Area.STORAGE.value(), String.format(Locale.US, "ResultCachingManager: getRequestResult() Found a cached result for Request %1$d", new Object[]{Integer.valueOf(operation.getRequest().getId())}));
    ...
    public void refreshCacheEntry(Operation operation) {
        if (operation == null) {
            ...
            try {
                ...
                Logger.i(Area.COMM.value() | Area.STORAGE.value(), String.format(Locale.US, "ResultCachingManager: refreshCacheEntry() Cache entry updated: %1$s", new Object[]{cacheEntry.toString()}));
            } catch (Exception e) {
            ...
            try {
                ...
                Logger.v(Area.COMM.value() | Area.STORAGE.value(), String.format(Locale.US, "Operation %1$d received a 304 and has been updated to a cached result with %2$d", new Object[]{Integer.valueOf(operation.getId()), Integer.valueOf(operation.getResult().getResponseCode())}));
