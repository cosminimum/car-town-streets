...
public class RequestLogger {
    ...
    private void resolveInstallationID() {
        ...
        Logger.d(Area.COMM.value(), String.format(Locale.US, "RequestLogger:_installation ID = '%1$s'", new Object[]{this._installationId}));
    }

    public void logRewardsWallShow() {
        if (_ExecutorService.getActiveCount() > 100) {
            Logger.w(Area.COMM.value(), String.format(Locale.US, "RequestLogger: queue of length %1$d exteeds max of %2$d", new Object[]{Integer.valueOf(_ExecutorService.getActiveCount()), 100}));
            return;
        }
    ...
    public void logRequestAfter(Operation operation, Integer responseCode, Integer executionTime, Exception exception, int reauthCount, int exceptionCount, String authState) {
        if (_ExecutorService.getActiveCount() > 100) {
            Logger.w(Area.COMM.value(), String.format(Locale.US, "RequestLogger: queue of length %1$d exceeds max of %2$d", new Object[]{Integer.valueOf(_ExecutorService.getActiveCount()), 100}));
            return;
        }
        final long timestamp = System.currentTimeMillis();
    ...
    public void pushLogMessage(Map<String, String> logValues) {
        try {
            ...
            if (httpResponse == null) {
                Logger.w(Area.COMM.value(), String.format(Locale.US, "RequestLogger: failed [URL:%1$s]", new Object[]{uriToUse}));
                return;
            }
            Logger.v(Area.COMM.value(), String.format(Locale.US, "RequestLogger: logged [URL:%1$s]", new Object[]{uriToUse}));
            Integer responseCode = null;
            if (httpResponse.getStatusLine() != null) {
                responseCode = Integer.valueOf(httpResponse.getStatusLine().getStatusCode());
            }
            if (responseCode != null) {
                Logger.d(Area.COMM.value(), String.format(Locale.US, "RequestLogger: result code: %1$d", new Object[]{responseCode}));
                return;
            }
