...
public class AuthManager {
    ...
    private void resetCurrentAuthFlow() {
        ...
        if (this._authFlowState.isAuthing()) {
            Logger.w(Area.AUTH.value(), String.format("resetCurrentAuthFlow(): Starting reset of auth from a state of '%1$s'", new Object[]{this._authFlowState.name()}));
            try {
                ...
                Logger.w(Area.AUTH.value(), String.format(str, new Object[]{this._authFlowState.name()}));
            }
        }
    }
    ...
    private boolean ensureAuthInternal(AuthUIParentInterface uiParent, ProviderHint providerHint, boolean reAuth) {
        boolean authStarted;
        Logger.d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() START [state:%1$s]", new Object[]{this._authFlowState.name()}));
        ...
        synchronized (this._authFlowLock) {
            if (!reAuth) {
                if (AuthFlowState.AUTHED.equals(this._authFlowState) && !this._authCachingManager.isAuthExpired()) {
                    Logger.i(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() finishing with state:%1$s", new Object[]{this._authFlowState.name()}));
                    authStarted = true;
                }
            }
            authStarted = false;
            if (!this._authFlowState.isAuthing()) {
                try {
                    ...
                    if (providers.getAppProvider() == null) {
                        Logger.e(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() Failed to resolved an AppAuthProvider to use %1$s", new Object[]{this._authFlowState.name()}));
                    } else if (providers.getUserProvider() == null) {
                        Logger.e(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() Failed to resolved a UserAuthProvider to use, state:%1$s", new Object[]{this._authFlowState.name()}));
                    } else if (!providers.getUserProvider().isUINeeded(this._commContext, authFlowId, providerHint) || uiParent != null) {
                        Logger.i(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() state:%1$s", new Object[]{this._authFlowState.name()}));
                        ...
                        Logger.w(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() 'uiParent' is NULL and %1$s currently requires UI, state:%2$s", new Object[]{providers.getUserProvider().getClass().getName(), this._authFlowState.name()}));
    ...
    public void notifyAuthCompleted(UserAuthResult authResult) {
        ...
        try {
            Logger.d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: notifyAuthCompleted() START [state:%1$s]", new Object[]{this._authFlowState.name()}));
            if (authResult == null) {
                authSucceeded = true;
            ...
            Logger.d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: notifyAuthCompleted() DONE [state:%1$s]", new Object[]{this._authFlowState.name()}));
        } catch (Throwable th) {
            synchronized (this._authFlowLock) {
                if (0 != 0) {
                    this._authFlowState = AuthFlowState.AUTHED;
    ...
    private class AuthFlow implements Runnable {
        ...
        public void run() {
            ...
            Logger.d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() START [state:%1$s]", new Object[]{AuthManager.this._authFlowState.name()}));
            UserAuthResult authResult2 = null;
            try {
                AuthFlowState unused = AuthManager.this._authFlowState = AuthFlowState.DATA_CHECKING;
                Logger.v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
                if (hasAuthData()) {
                ...
                Logger.d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() DONE %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
        ...
        private UserAuthResult performAuth(CommContext commContext, String authFlowId, AuthUIParentInterface uiParent, ProviderHint providerHint) {
            ...
            Logger.v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: AuthFlow.performAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
            ...
            Logger.v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.performAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
        ...
        private UserAuthResult validateAuth(CommContext commContext, String authFlowId, String authTokenToValidate) {
            ...
            try {
                ...
                Logger.v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.validateAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
                try {
                    ...
                    if (result == null) {
                        ...
                        try {
                            ...
                            Logger.e(Area.AUTH.value(), "AuthFlow: validateAuth() result.checkForBlacklistedOrUnsupported() failed", e2);
    ...
    public void waitOnAuth() {
        Logger.v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: waitOnAuth() START [stack:%1$s]", new Object[]{Logger.getShortStack()}));
        try {
            ...
            Logger.v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: waitOnAuth() FINISHED [stack:%1$s]", new Object[]{Logger.getShortStack()}));
            throw th;
        }
    ...
    public void waitOnAuthWithUI(String theTitle) {
        ...
        try {
            ...
            Logger.v(Area.AUTH.value() | Area.UI.value(), String.format(Locale.US, "UserAuth: waitOnAuthWithUI() waiting on authWithUiId: '%1$s'", new Object[]{authWithUiId}));
            ...
            Logger.v(Area.AUTH.value() | Area.UI.value(), String.format(Locale.US, "UserAuth: waitOnAuthWithUI() finished waiting on authWithUiId: '%1$s'", new Object[]{authWithUiId}));
    ...
    public void observeAuthWithUiId(String authWithUiId) {
        ...
        Logger.v(Area.AUTH.value(), String.format(Locale.US, "UserAuth: observeAuthWithUiId() observing authWithUiId: '%1$s'", new Object[]{authWithUiId}));
    ...
    private void reAuthInternal(AuthUIParentInterface uiParent) {
        String str;
        Logger.i(Area.AUTH.value(), String.format(Locale.US, "%1$s REAUTHORIZING", new Object[]{getLoggingPrefix()}));
        try {
            ensureAuthInternal(uiParent, (ProviderHint) null, true);
        } finally {
            str = "%1$s REAUTHORIZATION FINISHED";
            Logger.i(Area.AUTH.value(), String.format(Locale.US, str, new Object[]{getLoggingPrefix()}));
        }
    }
    ...
    private class AuthCachingManager {
        ...
        public String getAuthToken() {
            ...
            Logger.d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached authentication token value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }
        ...
        public String getUserAuthProviderFilter() {
            ...
            Logger.d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user auth provider filter value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }
        ...
        public String getUserAccessId() {
            ...
            Logger.d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user access ID value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }
        ...
        public String getUserDeviceId() {
            ...
            Logger.d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user device ID value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }
            ...
            if (claims == null) {
                ...
                for (String key : claims.keySet()) {
                    ...
                    if (!StringUtility.isNullOrEmpty(key) && !StringUtility.isNullOrEmpty(value)) {
                        ...
                        Logger.v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: claim added to cache [key:%1$s  value:%2$s  ttl:%3$d]", new Object[]{key, value, Long.valueOf(ttl)}));
                    }
                }
            }
        }
            ...
            if (settingsMap == null) {
                ...
                for (String key : settingsMap.keySet()) {
                    ...
                    if (!StringUtility.isNullOrEmpty(key)) {
                        ...
                        Logger.v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: setting added to cache [key:%1$s  value:%2$s  ttl:%3$d]", new Object[]{key, value, Long.valueOf(ttl)}));
                    }
                }
            }
        }
