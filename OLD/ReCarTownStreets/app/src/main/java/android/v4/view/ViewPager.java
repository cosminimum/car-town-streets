...
public class ViewPager extends ViewGroup {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i2 = 0; i2 < size2; i2++) {
            ...
            if (child2.getVisibility() != 8 && ((lp = (LayoutParams) child2.getLayoutParams()) == null || !lp.isDecor)) {
                child2.measure(MeasureSpec.makeMeasureSpec((int) (((float) childWidthSize) * lp.widthFactor), 1073741824), this.mChildHeightMeasureSpec);
            }
        }
    }
    ...
    private void recomputeScrollPosition(int width, int oldWidth, int margin, int oldMargin) {
        if (oldWidth <= 0 || this.mItems.isEmpty()) {
            ...
            int scrollPos = (int) (((float) width) * (ii != null ? Math.min(ii.offset, this.mLastOffset) : BitmapDescriptorFactory.HUE_RED));
            ...
            if (child2.getVisibility() != 8) {
                ...
                if (!lp2.isDecor && (ii = infoForChild(child2)) != null) {
                    ...
                    if (lp2.needsMeasure) {
                        ...
                        child2.measure(MeasureSpec.makeMeasureSpec((int) (((float) ((width - paddingLeft) - paddingRight)) * lp2.widthFactor), 1073741824), MeasureSpec.makeMeasureSpec((height - paddingTop) - paddingBottom, 1073741824));
                    }
        ...
        switch (action) {
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (dx != BitmapDescriptorFactory.HUE_RED && !isGutterDrag(this.mLastMotionX, dx)) {
                    ...
                    if (xDiff > ((float) this.mTouchSlop) && xDiff > yDiff) {
                        this.mIsBeingDragged = true;
                        setScrollState(1);
                        this.mLastMotionX = dx > BitmapDescriptorFactory.HUE_RED ? this.mInitialMotionX + ((float) this.mTouchSlop) : this.mInitialMotionX - ((float) this.mTouchSlop);
                        setScrollingCacheEnabled(true);
    ...
    public void fakeDragBy(float xOffset) {
        ...
        scrollTo((int) scrollX, getScrollY());
        pageScrolled((int) scrollX);
        MotionEvent ev = MotionEvent.obtain(this.mFakeDragBeginTime, SystemClock.uptimeMillis(), 2, this.mLastMotionX, BitmapDescriptorFactory.HUE_RED, 0);
        this.mVelocityTracker.addMovement(ev);
        ev.recycle();
    }
