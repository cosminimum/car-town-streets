        ...
        for (String cachedVideo : this.cachedVideos) {
            try {
                ...
                if (httpResponse == null) {
                    MMAdViewSDK.Log.m4416d("Pre cache worker: HTTP response is null");
                } else {
                    HttpEntity httpEntity = httpResponse.getEntity();
                    if (httpEntity == null) {
                        MMAdViewSDK.Log.m4416d("Pre cache worker: Null HTTP entity");
                    } else if (httpEntity.getContentLength() == 0) {
                        MMAdViewSDK.Log.m4416d("Pre cache worker: Millennial ad return failed. Zero content length returned.");
                    } else {
                            ...
                            try {
                                ...
                                if (videoAd == null) {
                                    ...
                                    try {
                                        ...
                                        e.printStackTrace();
                                        MMAdViewSDK.Log.m4420e("Pre cache worker interrupted: %s", e.getMessage());
                                    }
                                }
                            } catch (IllegalStateException e1) {
                                e1.printStackTrace();
                                MMAdViewSDK.Log.m4416d("Pre cache worker: Millennial ad return failed. Invalid response data.");
                ...
                MMAdViewSDK.Log.m4417d("Pre cache worker HTTP error: %s", e2.getMessage());
            }
        }
        synchronized (PreCacheWorker.class) {
            busy = false;
