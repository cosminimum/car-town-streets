    ...
    static final String navigateRedirects(String urlString) {
        ...
        while (!urlString.startsWith("https")) {
            try {
                ...
                MMAdViewSDK.Log.m4429v(LOG_URL_FORMAT, urlString);
            } catch (IOException | MalformedURLException e) {
                return urlString;
            } catch (SocketTimeoutException e2) {
                MMAdViewSDK.Log.m4416d("Connection timeout.");
                return urlString;
            } catch (URISyntaxException e3) {
                MMAdViewSDK.Log.m4416d("URI Syntax incorrect.");
                return urlString;
            }
        }
        return urlString;
    ...
    static Intent getIntentFromUri(Context context, Uri destinationUri, OverlaySettings settings) {
        ...
        if (!(destinationUri == null || destinationUri.getScheme() == null)) {
            if (destinationUri.getScheme().equalsIgnoreCase(MMAdViewSDK.Event.INTENT_MARKET)) {
                MMAdViewSDK.Log.m4428v("Creating Android Market intent.");
                ...
                MMAdViewSDK.Log.m4428v("Creating streaming video player intent.");
                ...
                MMAdViewSDK.Log.m4428v("Creating telephone intent.");
                ...
                MMAdViewSDK.Log.m4428v("Creating txt message intent.");
                ...
                MMAdViewSDK.Log.m4428v("Creating Google Maps intent.");
                intent = new Intent("android.intent.action.VIEW", destinationUri);
                ...
                MMAdViewSDK.Log.m4428v("Creating video player intent.");
                intent = new Intent(context, MMActivity.class);
                intent.setData(destinationUri);
                intent.putExtra("class", "com.millennialmedia.android.VideoPlayerActivity");
                ...
                if (settings == null || !settings.shouldLaunchToOverlay) {
                    MMAdViewSDK.Log.m4428v("Creating launch browser intent.");
                    intent = new Intent("android.intent.action.VIEW", destinationUri);
                    MMAdViewSDK.Event.intentStarted(context, (MMAdView) null, MMAdViewSDK.Event.INTENT_EXTERNAL_BROWSER);
                } else {
                    MMAdViewSDK.Log.m4428v("Creating launch overlay intent.");
                    intent = new Intent(context, MMActivity.class);
                ...
                MMAdViewSDK.Log.m4429v("Creating intent for unrecognized URI. %s", destinationUri);
                intent = new Intent("android.intent.action.VIEW", destinationUri);
            }
        }
        if (intent != null) {
            ...
            MMAdViewSDK.Log.m4429v("%s resolved to Intent: %s", destinationUri, intent);
        } else {
            MMAdViewSDK.Log.m4429v("%s", destinationUri);
        }
        return intent;
    }
            ...
            new Thread(new Runnable() {
                public void run() {
                    ...
                    if (destination == null || context == null || listener == null) {
                        if (listener != null) {
                            MMAdViewSDK.Log.m4429v("Could not start activity for %s", destinationUri);
                        ...
                        try {
                            ...
                            MMAdViewSDK.Log.m4420e("No activity found for %s", destinationUri);
                        }
                        ...
                        if (videoAd == null || !videoAd.canShow(context, (MMAdView) null, false)) {
                            MMAdViewSDK.Log.m4429v("mmvideo: Ad %s cannot be shown at this time.", id);
                            return;
                        }
                        if (str2 != null) {
                            HandShake.sharedHandShake(context).updateLastVideoViewedTime(context, str2);
                        }
                        MMAdViewSDK.Log.m4429v("mmvideo: attempting to play video %s", id);
