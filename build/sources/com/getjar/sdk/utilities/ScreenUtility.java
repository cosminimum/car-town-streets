...
public class ScreenUtility {
    ...
    public static JSONObject getDisplayMetrics(Context androidContext) {
        ...
        try {
            ...
            Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "ScreenUtility getDisplayMetrics() -- Error getting availableRes " + e1.getLocalizedMessage());
        }
        try {
            screenRes.put("width", screenData.getResolutionX());
            screenRes.put("height", screenData.getResolutionY());
        } catch (JSONException e12) {
            Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "ScreenUtility getDisplayMetrics() -- Error getting screenRes " + e12.getLocalizedMessage());
        }
        try {
            screenDpi.put(Constants.f677X, decimalFormat.format(screenData.getXDpi()));
            screenDpi.put(Constants.f678Y, decimalFormat.format(screenData.getYDpi()));
        } catch (JSONException e13) {
            Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "ScreenUtility getDisplayMetrics() -- Error getting screenDpi " + e13.getLocalizedMessage());
        }
        try {
            ...
            Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "ScreenUtility getDisplayMetrics() -- Error getting screenSize " + e14.getLocalizedMessage());
        }
        try {
            ...
            Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "ScreenUtility getDisplayMetrics() -- Error getting all json objects together " + e.getLocalizedMessage());
        }
        Logger.m640d(Area.UI.value() | Area.CONFIG.value(), "Screen details: " + output.toString());
        return output;
    }
    ...
    private static ScreenData getScreenData(Context androidContext) {
        ...
        double screenHeight = (double) (((float) displayMetrics.heightPixels) / displayMetrics.ydpi);
        return new ScreenData((double) displayMetrics.xdpi, (double) displayMetrics.ydpi, displayMetrics.widthPixels, displayMetrics.heightPixels, (double) (((float) displayMetrics.widthPixels) / displayMetrics.xdpi), screenHeight, availableResX, availableResY);
    }
}
