...
public class RewardUtility {
    ...
    public static void saveWebUrlData(Context context, String url) {
        if (context == null) {
            ...
            edit.commit();
            Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "Last known URL updated to '%1$s'", new Object[]{url}));
        }
    }
    ...
    public static int prepAppDataForReportUsage(String packageName, ReportUsageData.UsageType type, Context context, Map<String, String> appMetadata) {
        if (StringUtility.isNullOrEmpty(packageName)) {
            ...
            try {
                ...
                Logger.m648w(Area.CONFIG.value(), String.format(Locale.US, "Failed to get versionCode, versionName, and app flags [packageName:'%1$s' error:'%2$s']", new Object[]{packageName, e.getClass().getName()}));
                return 0;
            }
        }
    }
