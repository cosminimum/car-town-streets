...
public class AuthManager {
    ...
    public final AuthCachingManager _authCachingManager;
    private ManualResetEvent _authFlowEvent = new ManualResetEvent(false);
    private final Object _authFlowLock = new Object();
    /* access modifiers changed from: private */
    public volatile AuthFlowState _authFlowState = AuthFlowState.UNKNOWN;
    ...
    private AuthManager(Context androidContext) {
        ...
        this._authCachingManager = new AuthCachingManager();
        ...
        Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthManager initialized [userAuthProviderFilter:%1$s authToken:%2$s userAccessId:%3$s userDeviceId:%4$s authTTL:%5$d authTimestamp:%6$d]", new Object[]{this._userAuthProviderFilter, this._authToken, this._userAccessId, this._userDeviceId, Long.valueOf(this._authTTL), Long.valueOf(this._authTimestamp)}));
    }
    ...
    private void resetCurrentAuthFlow() {
        ...
        if (this._authFlowState.isAuthing()) {
            Logger.m648w(Area.AUTH.value(), String.format("resetCurrentAuthFlow(): Starting reset of auth from a state of '%1$s'", new Object[]{this._authFlowState.name()}));
            try {
                ...
                this._authTTL = 0;
                this._authTimestamp = 0;
                this._authFlowState = AuthFlowState.UNKNOWN;
                this._authFlowEvent.open();
            } finally {
                str = "resetCurrentAuthFlow(): Finished reset of auth to a state of '%1$s'";
                Logger.m648w(Area.AUTH.value(), String.format(str, new Object[]{this._authFlowState.name()}));
            }
        }
    }

    public boolean ensureAuth() {
        return ensureAuthInternal((AuthUIParentInterface) null, (ProviderHint) null, false);
    }

    public boolean ensureAuthWithUI(AuthUIParentInterface uiParent) {
        if (uiParent != null) {
            return ensureAuthInternal(uiParent, (ProviderHint) null, false);
        }
    ...
    public boolean ensureAuthResetCurrent(ProviderHint providerHint, boolean forceAuth) {
        ...
        return ensureAuthInternal((AuthUIParentInterface) null, providerHint, forceAuth);
    }

    private boolean ensureAuthInternal(AuthUIParentInterface uiParent, ProviderHint providerHint, boolean reAuth) {
        boolean authStarted;
        Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() START [state:%1$s]", new Object[]{this._authFlowState.name()}));
        ...
        synchronized (this._authFlowLock) {
            if (!reAuth) {
                if (AuthFlowState.AUTHED.equals(this._authFlowState) && !this._authCachingManager.isAuthExpired()) {
                    Logger.m644i(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() finishing with state:%1$s", new Object[]{this._authFlowState.name()}));
                    authStarted = true;
                }
            }
            authStarted = false;
            if (!this._authFlowState.isAuthing()) {
                try {
                    ...
                    Logger.m640d(value, String.format(locale, "AuthFlow: ensureAuthInternal() uiParent is %1$s, authFlowId: %2$s, reAuth: %3$s, called from [%4$s], %5$s", objArr));
                    ...
                    if (providers.getAppProvider() == null) {
                        Logger.m642e(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() Failed to resolved an AppAuthProvider to use %1$s", new Object[]{this._authFlowState.name()}));
                    } else if (providers.getUserProvider() == null) {
                        Logger.m642e(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() Failed to resolved a UserAuthProvider to use, state:%1$s", new Object[]{this._authFlowState.name()}));
                    } else if (!providers.getUserProvider().isUINeeded(this._commContext, authFlowId, providerHint) || uiParent != null) {
                        Logger.m644i(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() state:%1$s", new Object[]{this._authFlowState.name()}));
                        Logger.m644i(Area.AUTH.value(), "AuthFlow: ensureAuthInternal() [running auth-flow]");
                        ...
                        Logger.m648w(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: ensureAuthInternal() 'uiParent' is NULL and %1$s currently requires UI, state:%2$s", new Object[]{providers.getUserProvider().getClass().getName(), this._authFlowState.name()}));
                        Logger.m644i(Area.AUTH.value(), "AuthFlow: ensureAuthInternal() [unable to auth]");
                    }
                    if (!authStarted) {
    ...
    public void notifyAuthCompleted(UserAuthResult authResult) {
        ...
        try {
            Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: notifyAuthCompleted() START [state:%1$s]", new Object[]{this._authFlowState.name()}));
            if (authResult == null) {
                authSucceeded = true;
            ...
            Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: notifyAuthCompleted() DONE [state:%1$s]", new Object[]{this._authFlowState.name()}));
        } catch (Throwable th) {
            synchronized (this._authFlowLock) {
                if (0 != 0) {
                    this._authFlowState = AuthFlowState.AUTHED;
    ...
    private void updateSettingsDependentState() {
        try {
            ...
            Logger.m643e(Area.AUTH.value() | Area.CONFIG.value(), "updateSettingsDependentState() failed", e);
        }
    }

    private class AuthFlow implements Runnable {
        ...
        public void run() {
            ...
            Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() START [state:%1$s]", new Object[]{AuthManager.this._authFlowState.name()}));
            UserAuthResult authResult2 = null;
            try {
                AuthFlowState unused = AuthManager.this._authFlowState = AuthFlowState.DATA_CHECKING;
                Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
                if (hasAuthData()) {
                    AuthFlowState unused2 = AuthManager.this._authFlowState = AuthFlowState.EXPIRY_CHECKING;
                    Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
                    boolean isUINeeded = new AndroidAccountUserAuthProvider().isUINeeded(this._commContext, (String) null, this._providerHint);
                    if (isAuthExpired() || isUINeeded) {
                        if (!isUINeeded) {
                            authResult2 = validateAuth(this._commContext, this._authFlowId, this._authToken);
                        }
                        if (isUINeeded || !authResult2.getState().succeeded()) {
                            authResult2 = performAuth(this._commContext, this._authFlowId, this._uiParent, this._providerHint);
                        } else {
                            Logger.m640d(Area.AUTH.value(), "AuthFlow: AuthFlow.run() Auth is current and succeeded");
                        }
                    } else {
                        Logger.m640d(Area.AUTH.value(), "AuthFlow: AuthFlow.run() Auth is current and valid");
                    }
                } else {
                    authResult2 = performAuth(this._commContext, this._authFlowId, this._uiParent, this._providerHint);
                }
                Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.run() DONE %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
        ...
        private UserAuthResult performAuth(CommContext commContext, String authFlowId, AuthUIParentInterface uiParent, ProviderHint providerHint) {
            AuthFlowState unused = AuthManager.this._authFlowState = AuthFlowState.APP_AUTHING;
            Logger.m646v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: AuthFlow.performAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
            ...
            if (appResult == null || !appResult.getState().succeeded()) {
                Logger.m646v(Area.AUTH.value(), "AuthFlow: AuthFlow.performAuth() AppAuthProvider.authorizeApplication() failed");
            ...
            AuthFlowState unused2 = AuthManager.this._authFlowState = AuthFlowState.USER_AUTHING;
            Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.performAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
            return this._userProvider.ensureUser(appResult.getAuthToken(), commContext, authFlowId, uiParent, providerHint);
        }

        /* JADX INFO: finally extract failed */
        private UserAuthResult validateAuth(CommContext commContext, String authFlowId, String authTokenToValidate) {
            Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() START");
            try {
                AuthFlowState unused = AuthManager.this._authFlowState = AuthFlowState.VALIDATING;
                Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: AuthFlow.validateAuth() %1$s", new Object[]{AuthManager.this._authFlowState.name()}));
                try {
                    Result result = AuthorizationServiceProxy.getInstance().validateAuth(commContext, authFlowId, authTokenToValidate).get();
                    if (result == null) {
                        Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() Failed to get results, returning NULL");
                        UserAuthResult userAuthResult = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                        Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                        ...
                        try {
                            ...
                            Logger.m643e(Area.AUTH.value(), "AccountHistoryManager work failed", e);
                        }
                        ...
                        Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: validateAuth() DONE [userAccessId:%1$s, userDeviceId%2$s, authToken:%3$s, claimsCount:%4$d, ttl:%5$d]", new Object[]{authResult.getUserAccessId(), authResult.getUserDeviceId(), authResult.getAuthToken(), Integer.valueOf(authResult.getClaims().size()), Long.valueOf(authResult.getTTL())}));
                        Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                        return authResult;
                    } else {
                        try {
                            if (result.checkForBlacklistedOrUnsupported(commContext)) {
                                Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() We are blacklisted or unsupported");
                                UserAuthResult userAuthResult2 = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNSUPPORTED);
                                Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                                return userAuthResult2;
                            }
                            UserAuthResult userAuthResult3 = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                            Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                            return userAuthResult3;
                        } catch (JSONException e2) {
                            Logger.m643e(Area.AUTH.value(), "AuthFlow: validateAuth() result.checkForBlacklistedOrUnsupported() failed", e2);
                            UserAuthResult userAuthResult4 = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                            Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                            return userAuthResult4;
                        }
                    }
                } catch (InterruptedException e3) {
                    Logger.m643e(Area.AUTH.value(), "AuthFlow: validateAuth() opBaseAuth.get() failed", e3);
                    UserAuthResult userAuthResult5 = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                    Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                    return userAuthResult5;
                } catch (ExecutionException e4) {
                    Logger.m643e(Area.AUTH.value(), "AuthFlow: validateAuth() opBaseAuth.get() failed", e4);
                    UserAuthResult userAuthResult6 = new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                    Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                    return userAuthResult6;
                }
            } catch (Exception e5) {
                Logger.m643e(Area.AUTH.value(), "AuthFlow: validateAuth() failed", e5);
                Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
                return new UserAuthResult(this._userProvider.getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
            } catch (Throwable th) {
                Logger.m640d(Area.AUTH.value(), "AuthFlow: validateAuth() DONE");
        ...
        private boolean hasAuthData() {
            if (this._userAccessId == null || this._userDeviceId == null || this._authToken == null) {
                return false;
            }
            return true;
        }
    ...
    public void waitOnAuth() {
        Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: waitOnAuth() START [stack:%1$s]", new Object[]{Logger.getShortStack()}));
        try {
            this._authFlowEvent.waitForOpen();
        } catch (InterruptedException e) {
            Logger.m643e(Area.AUTH.value(), "AuthFlow: waitOnAuth() waitForOpen() failed", e);
            e.printStackTrace();
        } catch (Throwable th) {
            Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: waitOnAuth() FINISHED [stack:%1$s]", new Object[]{Logger.getShortStack()}));
            throw th;
        }
    ...
    public void waitOnAuthWithUI(String theTitle) {
        Logger.m646v(Area.AUTH.value() | Area.UI.value(), "UserAuth: waitOnAuthWithUI() START");
        try {
            waitOnAuth();
            Logger.m646v(Area.AUTH.value() | Area.UI.value(), "UserAuth: waitOnAuthWithUI() FINISHED");
        } catch (InterruptedException e) {
            throw new AuthException((Throwable) e);
        } catch (AuthException e2) {
            Logger.m648w(Area.AUTH.value() | Area.UI.value(), "UserAuth: waitOnAuthWithUI() waitOnAuth() failed");
            ...
            Logger.m646v(Area.AUTH.value() | Area.UI.value(), String.format(Locale.US, "UserAuth: waitOnAuthWithUI() waiting on authWithUiId: '%1$s'", new Object[]{authWithUiId}));
            ...
            Logger.m646v(Area.AUTH.value() | Area.UI.value(), String.format(Locale.US, "UserAuth: waitOnAuthWithUI() finished waiting on authWithUiId: '%1$s'", new Object[]{authWithUiId}));
    ...
    public void observeAuthWithUiId(String authWithUiId) {
        ...
        Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "UserAuth: observeAuthWithUiId() observing authWithUiId: '%1$s'", new Object[]{authWithUiId}));
    ...
    private class ProvidersBucket {
        ...
        public AppAuthProvider getAppProvider() {
    ...
    public void reAuth() {
        reAuthInternal((AuthUIParentInterface) null);
    }

    private void reAuthInternal(AuthUIParentInterface uiParent) {
        String str;
        Logger.m644i(Area.AUTH.value(), String.format(Locale.US, "%1$s REAUTHORIZING", new Object[]{getLoggingPrefix()}));
        try {
            ensureAuthInternal(uiParent, (ProviderHint) null, true);
        } finally {
            str = "%1$s REAUTHORIZATION FINISHED";
            Logger.m644i(Area.AUTH.value(), String.format(Locale.US, str, new Object[]{getLoggingPrefix()}));
        }
    }
    ...
    private static final String getLoggingPrefix() {
        ...
        return String.format(Locale.US, "AuthFlow: %1$s() [thread:%2$d]", new Object[]{methodName, Long.valueOf(Thread.currentThread().getId())});
    }

    private class AuthCachingManager {
        ...
        public String getAuthToken() {
            ...
            Logger.m640d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached authentication token value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }
        ...
        public boolean isAuthExpired() {
            ...
            if ((authTimestamp.longValue() + authTTL.longValue()) - 3600000 < System.currentTimeMillis()) {
                return true;
            }
            return false;
        }
        ...
        public void setAuthToken(String authToken, Long ttl) {
            this._authCachingManager.updateCache(_KeyAuthenticationToken, authToken, ttl, (String) null, (URI) null);
        }
        ...
        public String getUserAuthProviderFilter() {
            ...
            Logger.m640d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user auth provider filter value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }

        /* access modifiers changed from: private */
        public void setUserAuthProviderFilter(String userAuthProviderFilter, Long ttl) {
            this._authCachingManager.updateCache(_KeyUserAuthProviderFilter, userAuthProviderFilter, ttl, (String) null, (URI) null);
        }
        ...
        public String getUserAccessId() {
            ...
            Logger.m640d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user access ID value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }

        /* access modifiers changed from: private */
        public void setUserAccessId(String userAccessId, long ttl) {
            this._authCachingManager.updateCache(_KeyUserAccessID, userAccessId, Long.valueOf(ttl), (String) null, (URI) null);
        }
        ...
        public String getUserDeviceId() {
            ...
            Logger.m640d(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: Found cached user device ID value [%1$s]", new Object[]{entryValue}));
            return entryValue;
        }

        /* access modifiers changed from: private */
        public void setUserDeviceId(String userDeviceId) {
            this._authCachingManager.updateCache(_KeyUserDeviceID, userDeviceId, Long.MAX_VALUE, (String) null, (URI) null);
        }
            ...
            if (claims == null) {
                ...
                for (String key : claims.keySet()) {
                    ...
                    if (!StringUtility.isNullOrEmpty(key) && !StringUtility.isNullOrEmpty(value)) {
                        this._claimsAndCapsCachingManager.updateCache(key, value, Long.valueOf(ttl), (String) null, (URI) null);
                        Logger.m646v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: claim added to cache [key:%1$s  value:%2$s  ttl:%3$d]", new Object[]{key, value, Long.valueOf(ttl)}));
                    }
                }
            }
        }
            ...
            if (settingsMap == null) {
                ...
                for (String key : settingsMap.keySet()) {
                    ...
                    if (!StringUtility.isNullOrEmpty(key)) {
                        try {
                            this._settingsCachingManager.updateCache(key, Base64.encodeObject(value), Long.valueOf(ttl), (String) null, (URI) null);
                        } catch (IOException e) {
                            Logger.m649w(Area.AUTH.value() | Area.STORAGE.value(), "Error adding setting to cache", e);
                        }
                        Logger.m646v(Area.AUTH.value() | Area.STORAGE.value(), String.format(Locale.US, "AuthFlow: setting added to cache [key:%1$s  value:%2$s  ttl:%3$d]", new Object[]{key, value, Long.valueOf(ttl)}));
                    }
                }
            }
        }
        ...
        public Map<String, String> getCapabilities() {
            ...
            try {
                for (CacheEntry entry : this._claimsAndCapsCachingManager.getAllCacheEntries()) {
        ...
        public Map<String, SettingsValue> getSettings() {
            ...
            try {
                for (CacheEntry entry : this._settingsCachingManager.getAllCacheEntries()) {
                    try {
                        resultMap.put(entry.getName(), (SettingsValue) Base64.decodeToObject(entry.getValue()));
                    } catch (IOException e) {
                        Logger.m649w(Area.AUTH.value() | Area.STORAGE.value() | Area.CONFIG.value(), "AuthFlow: getSettings() failed", e);
                    } catch (ClassNotFoundException e2) {
                        Logger.m649w(Area.AUTH.value() | Area.STORAGE.value() | Area.CONFIG.value(), "AuthFlow: getSettings() failed", e2);
                    }
                }
                return Collections.unmodifiableMap(resultMap);
