...
public class ClaimsManager {
    ...
    public boolean canBuy() {
        if (!RewardUtility.checkPermission(this._context, _BillingPermission)) {
            Logger.m644i(Area.AUTH.value() | Area.BUY_GOLD.value(), "Auth: ClaimsManager: canBuy() returning FALSE");
            return false;
        }
        ...
        if (!checkBooleanClaim(caps, "user.currency.buy")) {
            ...
            Logger.m644i(Area.AUTH.value() | Area.BUY_GOLD.value(), "Auth: ClaimsManager: canBuy() returning FALSE [isBillingSupported]");
            return false;
        } else {
            Logger.m644i(Area.AUTH.value() | Area.BUY_GOLD.value(), "Auth: ClaimsManager: canBuy() returning TRUE");
            return true;
        }
    }
    ...
    public boolean canEarn() {
        ...
        if (!checkBooleanClaim(caps, "user.currency.earn")) {
            Logger.m644i(Area.AUTH.value() | Area.EARN.value(), "Auth: ClaimsManager: canEarn() returning FALSE");
            return false;
        } else if (!checkBooleanClaim(caps, "app.currency.earn")) {
            Logger.m644i(Area.AUTH.value() | Area.EARN.value(), "Auth: ClaimsManager: canEarn() returning FALSE");
            return false;
        } else {
            Logger.m644i(Area.AUTH.value() | Area.EARN.value(), "Auth: ClaimsManager: canEarn() returning TRUE");
            return true;
        }
    }
    ...
    public boolean canPurchaseManagedProducts() {
        ...
        if (!checkBooleanClaim(caps, "user.products.purchase")) {
            Logger.m644i(Area.AUTH.value() | Area.OFFER.value(), "Auth: ClaimsManager: canPurchaseManagedProducts() returning FALSE");
            return false;
            ...
            Logger.m644i(Area.AUTH.value() | Area.OFFER.value(), "Auth: ClaimsManager: canPurchaseManagedProducts() returning FALSE [isBillingSupported]");
            return false;
        }
    }
    ...
    public boolean canViewManagedProducts() {
        ...
        Logger.m644i(Area.AUTH.value() | Area.OFFER.value(), "Auth: ClaimsManager: canViewManagedProducts() returning FALSE");
        return false;
    }
    ...
    public boolean canUseUnmanagedLicenses() {
        ...
        if (!checkBooleanClaim(caps, "user.product_licenses.use")) {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canUseUnmanagedLicenses() returning FALSE");
            return false;
        } else if (!checkBooleanClaim(caps, "app.unmanaged_product_licenses.use")) {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canUseUnmanagedLicenses() returning FALSE");
            return false;
        } else {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canUseUnmanagedLicenses() returning TRUE");
            return true;
        }
    }
    ...
    public boolean canModifyUnmanagedLicenses() {
        ...
        if (!checkBooleanClaim(caps, "user.product_licenses.modify")) {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canModifyUnmanagedLicenses() returning FALSE");
            return false;
        } else if (!checkBooleanClaim(caps, "app.unmanaged_product_licenses.modify")) {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canModifyUnmanagedLicenses() returning FALSE");
            return false;
        } else {
            Logger.m644i(Area.AUTH.value() | Area.LICENSING.value(), "Auth: ClaimsManager: canModifyUnmanagedLicenses() returning TRUE");
            return true;
        }
    }
    ...
    public boolean canPurchaseUnmanagedProducts() {
        ...
        if (!checkBooleanClaim(caps, "app.unmanaged_products.purchase")) {
            Logger.m644i(Area.AUTH.value() | Area.PURCHASE.value(), "Auth: ClaimsManager: canPurchaseUnmanagedProducts() returning FALSE");
            return false;
        } else if (!checkBooleanClaim(caps, "user.products.purchase")) {
            Logger.m644i(Area.AUTH.value() | Area.PURCHASE.value(), "Auth: ClaimsManager: canPurchaseUnmanagedProducts() returning FALSE");
            return false;
        } else {
            Logger.m644i(Area.AUTH.value() | Area.PURCHASE.value(), "Auth: ClaimsManager: canPurchaseUnmanagedProducts() returning TRUE");
            return true;
        }
    }
            ...
            try {
                for (Map.Entry<String, String> cap : claims.entrySet()) {
                    if (key.equals(cap.getKey())) {
                        ...
                        Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "Auth: ClaimsManager: checkBooleanClaim() returning %1$s for %2$s", new Object[]{Boolean.valueOf(result), key}));
                        return result;
                    }
                }
            } catch (Exception e) {
                Logger.m643e(Area.AUTH.value(), "Auth: ClaimsManager: checkBooleanClaim() failed", e);
            }
            Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "Auth: ClaimsManager: checkBooleanClaim() returning false for %1$s", new Object[]{key}));
            return false;
        }
    }
}
