...
class AppAuthProvider implements AuthProvider {
    ...
    public AuthResult authorizeApplication(CommContext commContext, String authFlowId) {
        Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() START");
        try {
            try {
                ...
                if (result == null) {
                    Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() Failed to get results, returning NULL");
                    AuthResult authResult = new AuthResult(getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                    Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() DONE");
                    return authResult;
                    ...
                    Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "AuthFlow: authorizeApplication() DONE [authToken:%1$s, claimsCount:%2$d, ttl:%3$d]", new Object[]{authResult2.getAuthToken(), Integer.valueOf(authResult2.getClaims().size()), Long.valueOf(authResult2.getTTL())}));
                    Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() DONE");
                    return authResult2;
                } else {
                    try {
                        if (result.checkForBlacklistedOrUnsupported(commContext)) {
                            Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() We are blacklisted or unsupported");
                        ...
                        Logger.m643e(Area.AUTH.value(), "AuthFlow: authorizeApplication() result.checkForBlacklistedOrUnsupported() failed", e);
                ...
                Logger.m643e(Area.AUTH.value(), "AuthFlow: authorizeApplication() opBaseAuth.get() failed", e2);
                AuthResult authResult6 = new AuthResult(getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() DONE");
                return authResult6;
            } catch (ExecutionException e3) {
                Logger.m643e(Area.AUTH.value(), "AuthFlow: authorizeApplication() opBaseAuth.get() failed", e3);
                AuthResult authResult7 = new AuthResult(getProviderFilter(), AuthResult.State.UNKNOWN_FAILURE);
                Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() DONE");
                return authResult7;
            }
        } catch (Exception e4) {
            Logger.m643e(Area.AUTH.value(), "AuthFlow: authorizeApplication() failed", e4);
            Logger.m640d(Area.AUTH.value(), "AuthFlow: authorizeApplication() DONE");
