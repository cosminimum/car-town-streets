...
public class AccountHistoryDatabase extends SQLiteOpenHelper {
    ...
    public void ensureAccountEntry(String userAccessId, String userDeviceId, String accountName, String providerFilter, long timestamp) {
        Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.ensureAccountEntry() START");
        try {
                ...
                synchronized (this._databaseAccessLock) {
                    if (!checkForAccountEntry(userAccessId)) {
                        ...
                        if (getWritableDatabase().insert(DATABASE_TABLE_ACCOUNT_INFO, (String) null, values) >= 0) {
                            Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.ensureAccountEntry() inserted [userAccessId:%1$s userDeviceId:%2$s accountName:'%3$s' providerFilter:%4$s timestampLastAuth:%5$d timestampCreated:%6$d]", new Object[]{userAccessId, userDeviceId, accountName, providerFilter, Long.valueOf(timestamp), Long.valueOf(timestamp)}));
                        } else {
                            Logger.m642e(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.ensureAccountEntry() failed to insert [userAccessId:%1$s userDeviceId:%2$s accountName:'%3$s' providerFilter:%4$s timestampLastAuth:%5$d timestampCreated:%6$d]", new Object[]{userAccessId, userDeviceId, accountName, providerFilter, Long.valueOf(timestamp), Long.valueOf(timestamp)}));
                        }
                        trimLruEntriesAccountInfo();
                    }
                }
                Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.ensureAccountEntry() FINISHED");
            }
        } catch (Throwable th) {
            Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.ensureAccountEntry() FINISHED");
            throw th;
    ...
    public void insertEvent(String userAccessId, AccountEventType eventType, long timestamp) {
        Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.insertEvent() START");
        try {
            if (StringUtility.isNullOrEmpty(userAccessId)) {
                throw new IllegalArgumentException("'userAccessId' cannot be NULL or empty");
                ...
                synchronized (this._databaseAccessLock) {
                    ...
                    if (getWritableDatabase().insert(DATABASE_TABLE_ACCOUNT_EVENT, (String) null, values) >= 0) {
                        Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.insertEvent() inserted [userAccessId:%1$s eventType:%2$s timestamp:%3$d]", new Object[]{userAccessId, eventType.name(), Long.valueOf(timestamp)}));
                    } else {
                        Logger.m642e(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.insertEvent() failed to insert [userAccessId:%1$s eventType:%2$s timestamp:%3$d]", new Object[]{userAccessId, eventType.name(), Long.valueOf(timestamp)}));
                    }
                ...
                Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.insertEvent() FINISHED");
            }
        } catch (Throwable th) {
            Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.insertEvent() FINISHED");
            throw th;
    ...
    public java.util.List<com.getjar.sdk.comm.auth.AccountHistoryEvent> getEvents(java.lang.String r12) {
        /*
            ...
            com.getjar.sdk.logging.Logger.m646v(r0, r2)
        ...
        L_0x0020:
            ...
            com.getjar.sdk.logging.Logger.m646v(r1, r3)
            throw r0
        L_0x0034:
    ...
    private void updateAccountLastAuth(String userAccessId, long timestampLastAuth) {
        String str;
        Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), "AccountHistoryDatabase.updateAccountLastAuth() START");
        try {
            if (StringUtility.isNullOrEmpty(userAccessId)) {
                throw new IllegalArgumentException("'userAccessId' cannot be NULL or empty");
            }
            ...
            if (getWritableDatabase().update(DATABASE_TABLE_ACCOUNT_INFO, values, "userAccessId = ?", new String[]{userAccessId}) > 0) {
                Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.updateAccountLastAuth() updated [userAccessId:%1$s timestampLastAuth:%2$d]", new Object[]{userAccessId, Long.valueOf(timestampLastAuth)}));
            } else {
                Logger.m642e(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "AccountHistoryDatabase.updateAccountLastAuth() failed to update [userAccessId:%1$s timestampLastAuth:%2$d]", new Object[]{userAccessId, Long.valueOf(timestampLastAuth)}));
            }
        } finally {
            str = "AccountHistoryDatabase.updateAccountLastAuth() FINISHED";
            Logger.m646v(Area.STORAGE.value() | Area.AUTH.value(), str);
        }
    }
    ...
    private void trimLruEntries(String tableName, String orderBy, int lruCap) {
        if (getRecordCount(tableName) >= ((long) lruCap)) {
            ...
            try {
                ...
                Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "%1$d LRU rows deleted form '%2$s'", new Object[]{Integer.valueOf(count), tableName}));
            } finally {
                try {
                    results.close();
                } catch (Exception e) {
