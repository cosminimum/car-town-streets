...
public class ReportUsageProxy extends ServiceProxyBase {
    ...
    public Operation reportApplicationUsage(CommContext commContext, List<ReportUsageData> appUsageList) {
        if (commContext == null) {
            ...
            Logger.m640d(Area.USAGE.value() | Area.COMM.value(), String.format(Locale.US, "Filtered usage records to report from %1$d to %2$d", new Object[]{Integer.valueOf(appUsageList.size()), Integer.valueOf(filteredAppUsageList.size())}));
            ...
            while (appUsageItr.hasNext()) {
                try {
                    ...
                    Logger.m643e(Area.USAGE.value() | Area.COMM.value(), "appUsageDataToJson() failed", e);
                }
            }
            sb.append("]");
