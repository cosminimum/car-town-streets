...
public class UserAuth {
    ...
    private class EnsureUserAuthCallable implements Callable<User> {
        ...
        public User call() throws Exception {
            Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: call() START");
            try {
                AuthManager.initialize(UserAuth.this.commContext.getApplicationContext());
                Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: waitOnAuthWithUI() start");
                AuthManager.getInstance().waitOnAuthWithUI(this.theTitle);
                Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: waitOnAuthWithUI() finished");
                Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: Creating User instance");
                User user = new User();
                try {
                    ...
                    Logger.m643e(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: callback failed", e);
                }
                if (user == null) {
                    Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: call() FINISHED [returning null]");
                    return user;
                }
                Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: call() FINISHED [returning user instance]");
                return user;
            } catch (Exception e2) {
                Logger.m649w(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: failed", e2);
                try {
                    if (this.userAuthListener != null) {
                        this.userAuthListener.userAuthCompleted((User) null);
                    }
                } catch (Exception e3) {
                    Logger.m643e(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: callback failed", e3);
                }
                if (0 == 0) {
                    Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: call() FINISHED [returning null]");
                    return null;
                ...
                try {
                    ...
                    Logger.m643e(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: callback failed", e4);
                }
                if (0 == 0) {
                    Logger.m640d(Area.AUTH.value(), "UserAuth: EnsureUserAuthCallable: call() FINISHED [returning null]");
                } else {
