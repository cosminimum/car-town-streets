...
public class CachingManager {
    ...
    public void updateCache(String name, String value, Long ttl, String eTag, URI uri) {
        if (StringUtility.isNullOrEmpty(name)) {
            ...
            if (!this._dbCache.upsertCacheEntry(new CacheEntry(name, value, ttl, eTag, uri))) {
                Logger.m642e(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: updateCache() Cache entry update failed: %1$s.%2$s", new Object[]{this._dbCache.getDatabaseName(), name}));
                return;
            }
            Logger.m640d(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: updateCache() Cache entry updated: %1$s.%2$s", new Object[]{this._dbCache.getDatabaseName(), name}));
        }
    }
    ...
    public CacheEntry getUnexpiredCacheEntry(String name) {
        ...
        Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: getCurrentCacheEntry() Found a cache entry for %1$s.%2$s", new Object[]{this._dbCache.getDatabaseName(), name}));
        return cacheEntry;
    }
    ...
    public CacheEntry getCacheEntry(String name) {
        ...
        Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: getCacheEntry() Found a cache entry for %1$s.%2$s", new Object[]{this._dbCache.getDatabaseName(), name}));
        return cacheEntry;
    }
    ...
    public void removeCacheEntry(String name) {
        ...
        this._dbCache.deleteCacheEntry(name);
        Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: removeCacheEntry() CacheEntry \"%1$s\" removed", new Object[]{name}));
    }

    public void removeCacheEntries() {
        int deleteCount = this._dbCache.deleteCacheEntries();
        Logger.m646v(Area.STORAGE.value(), String.format(Locale.US, "CachingManager: removeCacheEntries() deleted %1$d entries", new Object[]{Integer.valueOf(deleteCount)}));
    }
