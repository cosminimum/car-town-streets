    ...
    public void sendUnsyncedData() {
        ...
        for (PhoneSessionEvent phoneSession : phoneSessions) {
            try {
                ...
                Logger.m643e(Area.USAGE.value(), "Bad phone session record loaded", e);
                try {
        ...
        for (ApplicationSessionEvent appSession : appSessions) {
            try {
                ...
                Logger.m643e(Area.USAGE.value(), "Bad application session record loaded", e3);
                try {
            ...
            for (ReportUsageData appUsage : usageList) {
                try {
                    ...
                    if (session instanceof PhoneSessionEvent) {
                        ...
                        Logger.m646v(Area.USAGE.value(), String.format(Locale.US, "Usage: UsageReporter: handleResults() Updated phone session record as synced [id:%1$d]", new Object[]{Long.valueOf(session.getId())}));
                    } else if (session instanceof ApplicationSessionEvent) {
                        usageDB.appSessionSetAsSynced(session.getId());
                        Logger.m646v(Area.USAGE.value(), String.format(Locale.US, "Usage: UsageReporter: handleResults() Updated application session record as synced [id:%1$d]", new Object[]{Long.valueOf(session.getId())}));
                    } else {
                    ...
                    Logger.m643e(Area.USAGE.value(), "Usage: UsageReporter: handleResults() Failed to find a Session for an App Usage", e3);
                }
            }
        }
    }
