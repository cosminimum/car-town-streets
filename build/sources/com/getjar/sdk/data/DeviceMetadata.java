...
public class DeviceMetadata {
    ...
    public DeviceMetadata(Context context) {
        ...
        if (StringUtility.isNullOrEmpty(serialNumber)) {
            Logger.m648w(Area.AUTH.value(), "Failed to get a serial number");
        }
        String androidId = findAndroidId(context);
        if (StringUtility.isNullOrEmpty(androidId)) {
            Logger.m648w(Area.AUTH.value(), "Failed to get the Android ID");
        }
        String deviceId = findDeviceId(context);
        if (StringUtility.isNullOrEmpty(androidId)) {
            Logger.m648w(Area.AUTH.value(), "Failed to get the device ID [TelephonyManager.getDeviceId()]");
        }
    ...
    private String findDeviceId(Context context) {
        ...
        if (!StringUtility.isNullOrEmpty(overrideDeviceID)) {
            Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "[*** OVERRIDE ***] Override value being used: 'identity.device.id' = '%1$s'", new Object[]{overrideDeviceID}));
    ...
    private String findSerialNo(Context context, int sdkLevel) {
        ...
        if (!StringUtility.isNullOrEmpty(overrideSerialNumber)) {
            Logger.m646v(Area.AUTH.value(), String.format(Locale.US, "[*** OVERRIDE ***] Override value being used: 'identity.serial.number' = '%1$s'", new Object[]{overrideSerialNumber}));
