...
public class Tracker {
    ...
    public void send(Map<String, String> params) {
        ...
        if (TextUtils.isEmpty(paramsToSend.get(Fields.TRACKING_ID))) {
            Log.m1077w(String.format("Missing tracking id (%s) parameter.", new Object[]{Fields.TRACKING_ID}));
        }
        String hitType = paramsToSend.get(Fields.HIT_TYPE);
        if (TextUtils.isEmpty(hitType)) {
            Log.m1077w(String.format("Missing hit type (%s) parameter.", new Object[]{Fields.HIT_TYPE}));
            hitType = "";
        }
        if (hitType.equals(HitTypes.TRANSACTION) || hitType.equals(HitTypes.ITEM) || tokensAvailable()) {
            this.mHandler.sendHit(paramsToSend);
        } else {
            Log.m1077w("Too many hits sent too quickly, rate limiting invoked.");
        }
    }
    ...
    public synchronized boolean tokensAvailable() {
        ...
        if (this.mTokens >= 2000) {
            ...
            Log.m1077w("Excessive tracking detected.  Tracking call ignored.");
            z = false;
        }
        return z;
    }
