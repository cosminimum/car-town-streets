...
class DelayedHitSender implements HitSender {
    ...
    public boolean sendHit(String url) {
        if (!this.mRateLimiter.tokenAvailable()) {
            Log.m4394w("Too many urls sent too quickly with the TagManagerSender, rate limiting invoked.");
            return false;
        }
        if (!(this.mWrapperUrl == null || this.mWrapperQueryParameter == null)) {
            try {
                url = this.mWrapperUrl + Utility.QUERY_START + this.mWrapperQueryParameter + "=" + URLEncoder.encode(url, "UTF-8");
                Log.m4392v("Sending wrapped url hit: " + url);
            } catch (UnsupportedEncodingException e) {
                Log.m4395w("Error wrapping URL for testing.", e);
                return false;
            }
        }
