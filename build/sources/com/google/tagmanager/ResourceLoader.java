...
class ResourceLoader implements Runnable {
    ...
    private boolean okToLoad() {
        ...
        Log.m4392v("...no network connectivity");
        return false;
    }
    ...
    private void loadResource() {
        ...
        Log.m4392v("Start loading resource from network ...");
        ...
        try {
            try {
                ...
                Log.m4392v("Successfully loaded resource: " + resource);
                if (!resource.hasResource()) {
                    Log.m4392v("No change for container: " + this.mContainerId);
                }
                this.mCallback.onSuccess(resource.hasResource() ? resource.getResource() : null);
                networkClient.close();
                Log.m4392v("Load resource from network finished.");
            } catch (IOException e) {
                Log.m4395w("Error when parsing downloaded resources from url: " + url + " " + e.getMessage(), e);
                this.mCallback.onFailure(LoadCallback.Failure.SERVER_ERROR);
            }
        } catch (FileNotFoundException e2) {
            Log.m4394w("No data is retrieved from the given url: " + url + ". Make sure container_id: " + this.mContainerId + " is correct.");
            this.mCallback.onFailure(LoadCallback.Failure.SERVER_ERROR);
        } catch (IOException e3) {
            Log.m4395w("Error when loading resources from url: " + url + " " + e3.getMessage(), e3);
    ...
    public void setCtfeURLPathAndQuery(String urlPathAndQuery) {
        ...
        Log.m4386d("Setting CTFE URL path: " + urlPathAndQuery);
    ...
    public void setPreviousVersion(String version) {
        Log.m4386d("Setting previous container version: " + version);
