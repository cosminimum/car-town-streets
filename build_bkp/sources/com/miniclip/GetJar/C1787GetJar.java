...
/* renamed from: com.miniclip.GetJar.GetJar */
public class C1787GetJar {
    ...
    static EnsureUserAuthListener userAuthListener = new EnsureUserAuthListener() {
        public void userAuthCompleted(User user) {
            User unused = C1787GetJar.mUser = user;
        }
    };

    /* renamed from: com.miniclip.GetJar.GetJar$GetJarListener */
    ...
    public static void getJarResponce(final int value) {
        mActivity.runOnUiThread(new Runnable() {
            public void run() {
                if (value == 1) {
                    AlertDialog dialog = new AlertDialog.Builder(C1787GetJar.mActivity).create();
    ...
    public static void createPickDialog(String itemID, int managed, String title, int showDialog) {
        ...
        if (items.containsKey(itemID)) {
            ...
            if (showDialog == 1) {
                ...
                mActivity.runOnUiThread(new Runnable() {
                    public void run() {
                        AlertDialog.Builder builder = new AlertDialog.Builder(C1787GetJar.mActivity);
                        ...
                        AlertDialog unused = C1787GetJar.dialog = builder.create();
                        C1787GetJar.dialog.show();
                    }
                });
                return;
            }
    ...
    /* renamed from: com.miniclip.GetJar.GetJar$PricingCallback */
    private static class PricingCallback implements RecommendedPricesListener {
        ...
        public void recommendedPricesEvent(RecommendedPrices recommendedPrices) {
            if (recommendedPrices != null) {
                for (Map.Entry<String, Integer> entry : C1787GetJar.items.entrySet()) {
                    ...
                    if (recommendedPrice != price) {
                        C1787GetJar.items.put(itemID, Integer.valueOf(recommendedPrice));
                    }
                }
            }
        }
