...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    public void dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            URL url = getUrl(hit);
            if (url == null) {
                Log.m4394w("No destination: discarding hit.");
                ...
                if (request == null) {
                    ...
                    if (firstSend) {
                        try {
                            ...
                            Log.m4394w("ClientProtocolException sending hit; discarding hit...");
                            this.dispatchListener.onHitPermanentDispatchFailure(hit);
                        } catch (IOException e2) {
                            Log.m4394w("Exception sending hit: " + e2.getClass().getSimpleName());
                            Log.m4394w(e2.getMessage());
                    ...
                    if (statusCode != 200) {
                        Log.m4394w("Bad response: " + response.getStatusLine().getStatusCode());
                        this.dispatchListener.onHitTransientDispatchFailure(hit);
                    } else {
    ...
    private HttpEntityEnclosingRequest constructGtmRequest(URL url) {
        ...
        try {
            ...
            try {
                ...
                e = e;
                request = request2;
                Log.m4394w("Exception sending hit: " + e.getClass().getSimpleName());
                Log.m4394w(e.getMessage());
                return request;
            }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m4392v("Error Writing hit to log...");
            }
        }
        Log.m4392v(httpHeaders.toString());
    }
    ...
    public URL getUrl(Hit hit) {
        try {
            ...
            Log.m4388e("Error trying to parse the GTM url.");
            return null;
        }
    }
}
