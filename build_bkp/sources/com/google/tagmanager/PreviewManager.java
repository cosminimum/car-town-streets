...
class PreviewManager {
        ...
        synchronized (this) {
            try {
                ...
                if (uriStr.matches(CONTAINER_PREVIEW_URL_PATTERN)) {
                    Log.m4392v("Container preview url: " + uriStr);
                    ...
                    Log.m4394w("Invalid preview uri: " + uriStr);
                    z = false;
                } else if (getContainerId(data.getQuery()).equals(this.mContainerId)) {
                    Log.m4392v("Exit preview mode for container: " + this.mContainerId);
    ...
    private String getContainerId(String query) {
        return query.split(Utility.QUERY_APPENDIX)[0].split("=")[1];
    }
}
