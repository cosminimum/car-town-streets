...
public class Container {
    ...
    public boolean getBoolean(String key) {
        ...
        if (runtime == null) {
            Log.m4388e("getBoolean called for closed container.");
        ...
        try {
            ...
            Log.m4388e("Calling getBoolean() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultBoolean().booleanValue();
        }
    }
    ...
    public double getDouble(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m4388e("getDouble called for closed container.");
        ...
        try {
            ...
            Log.m4388e("Calling getDouble() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultDouble().doubleValue();
        }
    }
    ...
    public long getLong(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m4388e("getLong called for closed container.");
            return Types.getDefaultInt64().longValue();
        }
        try {
            return Types.valueToInt64(runtime.evaluateMacroReference(key).getObject()).longValue();
        } catch (Exception e) {
            Log.m4388e("Calling getLong() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultInt64().longValue();
        }
    }
    ...
    public String getString(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m4388e("getString called for closed container.");
            return Types.getDefaultString();
        }
        try {
            return Types.valueToString(runtime.evaluateMacroReference(key).getObject());
        } catch (Exception e) {
            Log.m4388e("Calling getString() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultString();
        }
    }
    ...
    public synchronized void refresh() {
        if (getRuntime() == null) {
            Log.m4394w("refresh called for closed container");
        } else {
            try {
                if (isDefaultContainerRefreshMode()) {
                    Log.m4394w("Container is in DEFAULT_CONTAINER mode. Refresh request is ignored.");
                } else {
                    long currentTime = this.mClock.currentTimeMillis();
                    if (useAvailableToken(currentTime)) {
                        Log.m4392v("Container refresh requested");
                        loadAfterDelay(0);
                        this.mLastRefreshMethodCalledTime = currentTime;
                    } else {
                        Log.m4392v("Container refresh was called too often. Ignored.");
                    }
                }
            } catch (Exception e) {
                Log.m4388e("Calling refresh() throws an exception: " + e.getMessage());
            }
        }
        return;
    }
    ...
    public synchronized void close() {
        try {
            ...
            Log.m4388e("Calling close() threw an exception: " + e.getMessage());
        }
        this.mRuntime = null;
        return;
    }
    ...
    public synchronized void callRefreshSuccess(RefreshType refreshType) {
        Log.m4392v("calling containerRefreshSuccess(" + refreshType + "): mUserCallback = " + this.mUserCallback);
    ...
    public synchronized void load(Callback callback, ResourceLoaderScheduler scheduler, final Clock clock) {
        ...
        this.mDiskLoadCallback = new LoadCallback<Resource.ResourceWithMetadata>() {
            ...
            public void onSuccess(Resource.ResourceWithMetadata resource) {
                if (Container.this.isDefault()) {
                    ...
                    Log.m4392v("setting refresh time to saved time: " + resource.getTimeStamp());
            ...
            private RefreshFailure failureToRefreshFailure(LoadCallback.Failure failure) {
                switch (C16474.$SwitchMap$com$google$tagmanager$LoadCallback$Failure[failure.ordinal()]) {
                    case 1:
        ...
        if (isDefaultContainerRefreshMode()) {
            Log.m4390i("Container is in DEFAULT_CONTAINER mode. Use default container.");
        } else {
            ...
            this.mNetworkLoadCallback = new LoadCallback<Serving.Resource>() {
                ...
                public void onSuccess(Serving.Resource resource) {
                    synchronized (Container.this) {
                        ...
                        Log.m4392v("setting refresh time to current time: " + Container.this.mLastRefreshTime);
    ...
    public void initEvaluators(Serving.Resource resource, boolean isDefaultResource) {
        try {
            ...
            Log.m4388e("Not loading resource: " + resource + " because it is invalid: " + err.toString());
        }
    }
    ...
    private void createInitialContainer() {
        ...
        if (expandedResource == null) {
            Log.m4394w("No default container found; creating an empty container.");
