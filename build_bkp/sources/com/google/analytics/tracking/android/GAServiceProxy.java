...
class GAServiceProxy implements ServiceProxy, AnalyticsGmsCoreClient.OnConnectedListener, AnalyticsGmsCoreClient.OnConnectionFailedListener {
    ...
    public synchronized void setForceLocalDispatch() {
        if (!this.forceLocalDispatch) {
            Log.m1076v("setForceLocalDispatch called.");
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:16:0x003e, code lost:
        r6 = r7.queue.poll();
        com.google.analytics.tracking.android.Log.m1076v("Sending hit to store  " + r6);
    ...
    public synchronized void sendQueue() {
        ...
        L_0x0028:
            int[] r0 = com.google.analytics.tracking.android.GAServiceProxy.C05793.f1106x3c5f147c     // Catch:{ all -> 0x0072 }
        ...
        L_0x0036:
            ...
            com.google.analytics.tracking.android.Log.m1076v(r0)     // Catch:{ all -> 0x0072 }
    ...
    public synchronized void useStore() {
        if (this.state != ConnectState.CONNECTED_LOCAL) {
            clearAllTimers();
            Log.m1076v("falling back to local store");
    ...
    public synchronized void connectToService() {
        if (this.forceLocalDispatch || this.client == null || this.state == ConnectState.CONNECTED_LOCAL) {
            Log.m1077w("client not initialized.");
            useStore();
        } else {
            try {
                this.connectTries++;
                ...
                Log.m1076v("connecting to Analytics service");
                this.client.connect();
            } catch (SecurityException e) {
                Log.m1077w("security exception on connectToService");
                useStore();
            }
        }
        return;
    ...
    public synchronized void onConnected() {
        this.failedConnectTimer = cancelTimer(this.failedConnectTimer);
        this.connectTries = 0;
        Log.m1076v("Connected to service");
    ...
    public synchronized void onDisconnected() {
        if (this.state == ConnectState.PENDING_DISCONNECT) {
            Log.m1076v("Disconnected from service");
            clearAllTimers();
            this.state = ConnectState.DISCONNECTED;
        } else {
            Log.m1076v("Unexpected disconnect.");
    ...
    public synchronized void onConnectionFailed(int errorCode, Intent resolution) {
        this.state = ConnectState.PENDING_CONNECTION;
        if (this.connectTries < 2) {
            Log.m1077w("Service unavailable (code=" + errorCode + "), will retry.");
            fireReconnectAttempt();
        } else {
            Log.m1077w("Service unavailable (code=" + errorCode + "), using local store.");
            useStore();
        }
    }
    ...
    private class DisconnectCheckTask extends TimerTask {
        ...
        public void run() {
            ...
            Log.m1076v("Disconnecting due to inactivity");
