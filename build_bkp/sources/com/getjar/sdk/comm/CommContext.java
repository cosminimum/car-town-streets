...
public class CommContext {
    ...
    protected CommContext(String applicationKey, Context context, ResultReceiver resultReceiver) {
        ...
        if (StringUtility.isNullOrEmpty(applicationKey)) {
            ...
            Logger.m646v(Area.COMM.value(), "CommContext: creating DeviceMetadata instance");
            this._deviceMetadata = new DeviceMetadata(context);
            this._applicationContext = context.getApplicationContext();
            Logger.m646v(Area.COMM.value(), "CommContext: calling UserAgentValuesManager.getSdkUserAgent()");
    ...
    private void setResponse(Response response) {
        ...
        synchronized (this._pulledResponseLock) {
            if (this._pulledResponse == null) {
                ...
                Logger.m646v(Area.COMM.value(), String.format(Locale.US, "Current response updated to %1$s", new Object[]{this._pulledResponse.getClass().getName()}));
            } else {
                ...
                Logger.m646v(Area.COMM.value(), String.format(Locale.US, "setResponse() called [currentResponse:%1$d:%2$s] [newResponse:%3$d:%4$s]", new Object[]{Integer.valueOf(currentResponseRank), this._pulledResponse.getClass().getName(), Integer.valueOf(newResponseRank), response.getClass().getName()}));
    ...
    public void makeServiceFailureCallbacks(Result result) {
        Logger.m640d(Area.COMM.value(), String.format(Locale.US, "makeServiceFailureCallbacks() with %1$d call-back interfaces registered [from %2$s]", new Object[]{Integer.valueOf(this._commFailureCallbackInterfaceList.size()), Thread.currentThread().getStackTrace()[3].getMethodName()}));
    ...
    public void makeNetworkFailureCallbacks() {
        Logger.m640d(Area.COMM.value(), String.format(Locale.US, "makeNetworkFailureCallbacks() with %1$d call-back interfaces registered [from %2$s]", new Object[]{Integer.valueOf(this._commFailureCallbackInterfaceList.size()), Thread.currentThread().getStackTrace()[3].getMethodName()}));
        Iterator i$ = this._commFailureCallbackInterfaceList.iterator();
        while (i$.hasNext()) {
            try {
    ...
    public void makeAuthorizationFailureCallbacks(String subcode) {
        Logger.m640d(Area.AUTH.value(), String.format(Locale.US, "makeAuthorizationFailureCallbacks() with %1$d call-back interfaces registered [from %2$s]", new Object[]{Integer.valueOf(this._commFailureCallbackInterfaceList.size()), Thread.currentThread().getStackTrace()[3].getMethodName()}));
        Iterator i$ = this._commFailureCallbackInterfaceList.iterator();
        while (i$.hasNext()) {
            try {
    ...
    public void clearResponse() {
        ...
        Logger.m646v(Area.COMM.value(), "Current response cleared");
    }
