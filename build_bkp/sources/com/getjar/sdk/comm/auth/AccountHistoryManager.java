    ...
    public String getCurrentAccountName() {
        for (AccountHistoryEvent event : AccountHistoryDatabase.getInstance(this._context).getEvents()) {
            if (event.getEventType().isAuthEvent() || AccountEventType.USER_SWITCHED.equals(event.getEventType())) {
                ...
                Logger.m642e(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "getCurrentAccountName() Failed to load an account info record for user access ID '%1$s'", new Object[]{event.getUserAccessId()}));
            }
        }
        return null;
    }
    ...
    public String getPreviousAccountName() {
        ...
        for (AccountHistoryEvent event : AccountHistoryDatabase.getInstance(this._context).getEvents()) {
            if (currentUserAccessId == null) {
                ...
                Logger.m642e(Area.STORAGE.value() | Area.AUTH.value(), String.format(Locale.US, "getPreviousAccountName() Failed to load an account info record for user access ID '%1$s'", new Object[]{event.getUserAccessId()}));
            }
        }
        return null;
    }
