...
public final class RequestUtilities {
    public static ServiceException getServicesException(Result result) throws ServiceException, JSONException {
        ...
        if (result.getResponseJson() != null) {
            if (result.getResponseJson().has(GCMConstants.EXTRA_ERROR)) {
                return new ServiceException(String.format(Locale.US, "[ResponseCode: %1$d] Result: %2$s", new Object[]{Integer.valueOf(result.getResponseCode()), result.getResponseJson().toString(2)}), result);
            } else if (!result.getResponseJson().has("return")) {
                return new ServiceException(String.format(Locale.US, "Unexpected JSON result [ResponseCode: %1$d] Result: %2$s", new Object[]{Integer.valueOf(result.getResponseCode()), result.getResponseJson().toString(2)}), result);
            }
        }
        return null;
    }

    protected static void debugDumpRequestProperties(HttpRequestBase httpRequest) {
        try {
            Logger.m640d(Area.COMM.value(), "The request properties for this request:");
            for (Header header : httpRequest.getAllHeaders()) {
                Logger.m640d(Area.COMM.value(), "      " + header.getName() + " = '" + header.getValue() + "'");
            }
        } catch (Throwable t) {
            t.printStackTrace();
        }
    ...
    protected static String getPostDataBlob(Map<String, String> postData) {
    ...
    protected static boolean checkForRetryOnException(Exception theException, boolean forIdempotentOperation) {
