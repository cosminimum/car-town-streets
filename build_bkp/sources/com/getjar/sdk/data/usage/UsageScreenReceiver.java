...
public class UsageScreenReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            Logger.m640d(Area.OS_ENTRY_POINT.value(), "UsageScreenReceiver onReceive start()");
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        ...
                        Logger.m643e(Area.USAGE.value() | Area.OS_ENTRY_POINT.value(), "UsageScreenReceiver: onReceive() failed", e);
                    }
                }
    ...
    public void doOnReceive(Context context, Intent intent) {
        if (intent.getAction().equals("android.intent.action.SCREEN_OFF")) {
            Logger.m644i(Area.USAGE.value(), "Usage: UsageScreenReceiver: screen off");
            UsageManager.getInstance(context).stopPhoneSession();
        } else if (intent.getAction().equals("android.intent.action.SCREEN_ON")) {
            Logger.m644i(Area.USAGE.value(), "Usage: UsageScreenReceiver: screen on");
    ...
    public boolean isScreenOn(Context context) {
        try {
            ...
            Logger.m643e(Area.USAGE.value(), "Usage: UsageScreenReceiver: isScreenOn() failed", e);
            return true;
        }
    }
    ...
    public synchronized void registerReceiver(Context context) {
        if (!this._isRegistered) {
            ...
            Logger.m646v(Area.USAGE.value(), "Usage: UsageScreenReceiver: screen monitor registered");
        }
    }
    ...
    public synchronized void unregisterReceiver(Context context) {
        if (this._isRegistered) {
            ...
            Logger.m646v(Area.USAGE.value(), "Usage: UsageScreenReceiver: screen monitor unregistered");
        }
    }
}
