...
public class EarningScreenReceiver extends BroadcastReceiver {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        ...
                        Logger.m643e(Area.EARN.value() | Area.TRANSACTION.value(), "EarningScreenReceiver: onReceive() failed", e);
                    }
                }
    ...
    public void doOnReceive(Context context, Intent intent) {
        try {
            if (intent.getAction().equals("android.intent.action.SCREEN_OFF")) {
                Logger.m644i(Area.EARN.value() | Area.TRANSACTION.value(), "Earning: EarningScreenReceiver: screen off");
                EarningMonitor.getInstance(context).stopMonitoring();
            } else if (intent.getAction().equals("android.intent.action.SCREEN_ON")) {
                Logger.m644i(Area.EARN.value() | Area.TRANSACTION.value(), "Earning: EarningScreenReceiver: screen on");
                EarningMonitor.getInstance(context).startMonitoring();
            }
        } catch (Exception e) {
            Logger.m643e(Area.EARN.value() | Area.TRANSACTION.value(), "EarningScreenReceiver: doOnReceive() failed", e);
        }
    }
    ...
    public synchronized void registerReceiver(Context context) {
        if (!this._isRegistered) {
            ...
            Logger.m646v(Area.EARN.value() | Area.TRANSACTION.value(), "Earning: EarningScreenReceiver: screen monitor registered");
        }
    }
    ...
    public synchronized void unregisterReceiver(Context context) {
        if (this._isRegistered) {
            ...
            Logger.m646v(Area.EARN.value() | Area.TRANSACTION.value(), "Earning: EarningScreenReceiver: screen monitor unregistered");
        }
    }
}
