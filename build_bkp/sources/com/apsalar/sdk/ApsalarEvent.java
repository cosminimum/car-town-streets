...
class ApsalarEvent implements ApsalarAPI, ApsalarJSON {
    ...
    protected String urlbase = "http://e.apsalar.com/api/v1";
    ...
    protected ApsalarEvent() {
        init();
    }

    protected ApsalarEvent(ApsalarSessionInfo apsalarSessionInfo) {
        init();
    ...
    protected ApsalarEvent(ApsalarSessionInfo apsalarSessionInfo, String str, String str2) {
        init();
        this.info = apsalarSessionInfo;
        this.eventName = str;
        this.eventData = str2;
    ...
    protected ApsalarEvent(ApsalarSessionInfo apsalarSessionInfo, JSONObject jSONObject) {
        init();
        this.info = apsalarSessionInfo;
        try {
    ...
    public void makeURL() {
        String str = "";
        try {
            str = "?a=" + URLEncoder.encode(this.info.apiKey, "UTF-8") + "&av=" + URLEncoder.encode(this.info.appVersion, "UTF-8") + "&e=" + URLEncoder.encode(this.eventData, "UTF-8") + "&i=" + URLEncoder.encode(this.info.clsPackage, "UTF-8") + "&n=" + URLEncoder.encode(this.eventName, "UTF-8") + "&p=" + URLEncoder.encode(this.info.platform, "UTF-8") + "&rt=" + URLEncoder.encode(this.info.retType, "UTF-8") + "&s=" + URLEncoder.encode(this.info.sessionId, "UTF-8") + "&sdk=" + URLEncoder.encode(this.info.sdkVersion, "UTF-8") + "&t=" + (((double) (this.eventTime - this.info.sessionStart)) * 0.001d) + "&u=" + URLEncoder.encode(this.info.deviceId, "UTF-8");
    ...
    public int REST(Boolean bool) {
        makeURL();
        String str = this.url + "&lag=" + (((double) (System.currentTimeMillis() - this.eventTime)) * 0.001d);
        try {
            MessageDigest instance = MessageDigest.getInstance("SHA-1");
            instance.update(this.info.secret.getBytes("UTF-8"));
            instance.update(str.getBytes("UTF-8"));
            try {
                try {
                    this.returnData = (String) client.execute(new HttpGet(this.urlbase + str + "&h=" + Apsalar.hexDigest(instance.digest())), new BasicResponseHandler());
