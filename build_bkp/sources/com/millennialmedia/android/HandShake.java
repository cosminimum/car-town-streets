...
class HandShake {
    ...
    static synchronized HandShake sharedHandShake(Context context) {
        ...
        synchronized (HandShake.class) {
            if (apid == null) {
                MMAdViewSDK.Log.m4419e("No apid set for the handshake.");
                handShake = null;
            } else {
                if (sharedInstance == null) {
                    ...
                    MMAdViewSDK.Log.m4416d("Handshake expired, requesting new handshake from the server.");
                    sharedInstance = new HandShake(context);
                }
                ...
                public void run() {
                    ...
                    if (tempContext != null) {
                        try {
                            ...
                            MMAdViewSDK.Log.m4429v("Performing handshake: %s", handshakeUrl);
                            ...
                            MMAdViewSDK.Log.m4420e("Could not get a handshake. %s", e.getMessage());
                        }
                    }
                }
            }).start();
            ...
            while (i$.hasNext()) {
                ...
                if (scheme.checkAvailability(context)) {
                    if (stringBuilder.length() > 0) {
                        stringBuilder.append("," + scheme.f3959id);
                    } else {
                        stringBuilder.append(Integer.toString(scheme.f3959id));
                    }
                }
            }
            if (stringBuilder.length() > 0) {
            ...
            while (i$.hasNext()) {
                ...
                if (scheme.checkAvailability(context)) {
                    try {
                        ...
                        schemeObject.put("schemeid", scheme.f3959id);
    ...
    private class Scheme {
        ...
        public void deserializeFromObj(JSONObject schemeObject) {
            if (schemeObject != null) {
                ...
                this.f3959id = schemeObject.optInt("schemeid");
            }
        }
    }
    ...
    private class AdTypeHandShake {
        ...
        public boolean canRequestVideo(Context context, String adType) {
            MMAdViewSDK.Log.m4417d("canRequestVideo() Current Time: %d Last Video: %d  Diff: %d  Video interval: %d", Long.valueOf(System.currentTimeMillis()), Long.valueOf(this.lastVideo / 1000), Long.valueOf((System.currentTimeMillis() - this.lastVideo) / 1000), Long.valueOf(this.videoInterval / 1000));
    ...
    public JSONObject parseJson(String jsonString) {
        MMAdViewSDK.Log.m4417d("JSON String: %s", jsonString);
        if (jsonString != null) {
            try {
                JSONObject jsonObject = new JSONObject(jsonString);
                MMAdViewSDK.Log.m4428v(jsonObject.toString());
                        ...
                        if (jsonObject != null) {
                            ...
                            if (!(message == null || type == null)) {
                                if (type.equalsIgnoreCase("log")) {
                                    MMAdViewSDK.Log.m4419e(message);
    ...
    private boolean loadHandShake(Context context) {
        ...
        if (settingsFound) {
            MMAdViewSDK.Log.m4416d("Handshake successfully loaded from shared preferences.");
    ...
    public void saveHandShake(Context context) {
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < this.schemes.size(); i++) {
                ...
                stringBuilder.append(scheme.scheme + "\t" + scheme.f3959id);
            }
