...
public class Facebook {
    ...
    private void authorize(Activity activity, String[] permissions, int activityCode, SessionLoginBehavior behavior, final DialogListener listener) {
        ...
        Session.OpenRequest openRequest = new Session.OpenRequest(activity).mo19setCallback(callback).mo21setLoginBehavior(behavior).mo23setRequestCode(activityCode).mo22setPermissions(Arrays.asList(permissions));
        ...
        do {
            synchronized (this.lock) {
                if (this.userSetSession != null) {
                    ...
                    Session.OpenRequest openRequest = new Session.OpenRequest(this.pendingAuthorizationActivity).mo22setPermissions(permissions);
    ...
    public void setAccessExpiresIn(String expiresInSecsFromNow) {
        ...
        if (expiresInSecsFromNow != null) {
            long expires = expiresInSecsFromNow.equals("0") ? 0L : System.currentTimeMillis() + (Long.parseLong(expiresInSecsFromNow) * 1000);
