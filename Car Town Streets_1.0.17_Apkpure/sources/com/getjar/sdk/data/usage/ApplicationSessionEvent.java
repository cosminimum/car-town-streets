...
public class ApplicationSessionEvent extends SessionEvent {
    ...
    public String toString() {
        return String.format(Locale.US, "id:%1$d packageName:%2$s timestamp:%3$d type:%4$s reason:%5$s reasonDetails:%6$s sessionId:%7$s phoneSessionId:%8$s synced:%9$s disposable:%10$s", Long.valueOf(getId()), getPackageName(), Long.valueOf(getTimestamp()), getType().name(), getReason().name(), getReasonDetails(), getSessionId(), getPhoneSessionId(), Boolean.valueOf(isSynced()), Boolean.valueOf(isDisposable()));
    }
}
