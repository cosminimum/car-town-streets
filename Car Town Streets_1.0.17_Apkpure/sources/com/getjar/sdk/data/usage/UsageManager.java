...
public class UsageManager {
    ...
    private static Comparator<AggregateUsageRecord> ComparatorPackageName = new Comparator<AggregateUsageRecord>() { // from class: com.getjar.sdk.data.usage.UsageManager.1
    ...
    private static Comparator<AggregateUsageRecord> ComparatorTimestampStart = new Comparator<AggregateUsageRecord>() { // from class: com.getjar.sdk.data.usage.UsageManager.2
        @Override // java.util.Comparator
        public int compare(AggregateUsageRecord lhs, AggregateUsageRecord rhs) {
            return (int) (rhs.getTimestampStart() - lhs.getTimestampStart());
        }
    };
    private static Comparator<AggregateUsageRecord> ComparatorTimestampStop = new Comparator<AggregateUsageRecord>() { // from class: com.getjar.sdk.data.usage.UsageManager.3
        @Override // java.util.Comparator
        public int compare(AggregateUsageRecord lhs, AggregateUsageRecord rhs) {
            return (int) (rhs.getTimestampStop() - lhs.getTimestampStop());
        }
    };
    private static Comparator<AggregateUsageRecord> ComparatorTotalUseDuration = new Comparator<AggregateUsageRecord>() { // from class: com.getjar.sdk.data.usage.UsageManager.4
        @Override // java.util.Comparator
        public int compare(AggregateUsageRecord lhs, AggregateUsageRecord rhs) {
            return rhs.getTotalUseDuration() - lhs.getTotalUseDuration();
        }
    };
    private static Comparator<AggregateUsageRecord> ComparatorTotalSessionsCount = new Comparator<AggregateUsageRecord>() { // from class: com.getjar.sdk.data.usage.UsageManager.5
        @Override // java.util.Comparator
        public int compare(AggregateUsageRecord lhs, AggregateUsageRecord rhs) {
    ...
    public boolean isMonitoringEnabled() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager isMonitoringEnabled: %1$s", value.toString()));
    ...
    private boolean getBackgroundSendEnabled() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getBackgroundSendEnabled: %1$s", value.toString()));
        return value.booleanValue();
    }
    ...
    private boolean getRequestSendEnabled() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getRequestSendEnabled: %1$s", value.toString()));
        return value.booleanValue();
    }
    ...
    public int getBackgroundBatchCount() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getBackgroundBatchCount: %1$s", value.toString()));
        return value.intValue();
    }

    private String getFilterRegex() {
        String value = GetJarConfig.getInstance(this._context).getDirectiveValue(GetJarConfig.KEY_USAGE_PACKAGE_FILTER_REGEX, null);
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getFilterRegex: %1$s", value));
        return value;
    }

    private String getBackgroundTypeFilter() {
        String value = GetJarConfig.getInstance(this._context).getDirectiveValue(GetJarConfig.KEY_USAGE_BACKGROUND_TYPE_FILTER, null);
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getBackgroundTypeFilter: %1$s", value));
        return value;
    }
    ...
    private boolean isFilterSystemEnabled() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager isFilterSystemEnabled: %1$s", value.toString()));
        return value.booleanValue();
    }

    public int getBackgroundSendIntervalMilliseconds() {
        Integer value = GetJarConfig.getInstance(this._context).getIntegerValue(GetJarConfig.KEY_USAGE_BACKGROUND_SEND_INTERVAL, Integer.valueOf((int) BACKGROUND_SEND_INTERVAL_SECONDS));
        if (value == null) {
            value = Integer.valueOf((int) BACKGROUND_SEND_INTERVAL_SECONDS);
        }
        Integer value2 = Integer.valueOf(value.intValue() * 1000);
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getBackgroundSendIntervalMilliseconds: %1$s", value2.toString()));
        return value2.intValue();
    }

    public int getRequestTimeWindowMilliseconds() {
        Integer value = GetJarConfig.getInstance(this._context).getIntegerValue(GetJarConfig.KEY_USAGE_REQUEST_TIME_WINDOW, Integer.valueOf((int) REQUEST_TIME_WINDOW_SECONDS));
        if (value == null) {
            value = Integer.valueOf((int) REQUEST_TIME_WINDOW_SECONDS);
        }
        Integer value2 = Integer.valueOf(value.intValue() * 1000);
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getRequestTimeWindowMilliseconds: %1$s", value2.toString()));
        return value2.intValue();
    }
    ...
    public int getRequestTimeWindowCount() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getRequestTimeWindowCount: %1$s", value.toString()));
        return value.intValue();
    }
    ...
    private int getRequestMaxCount() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getRequestMaxCount: %1$s", value.toString()));
        return value.intValue();
    }
    ...
    private String getRequestSort() {
        ...
        Logger.d(Area.USAGE.value() | Area.CONFIG.value(), String.format("UsageManager getRequestSort: %1$s", value));
        return value;
    }
    ...
    public long startAppSession(String packageName, SessionEvent.Reason reason, String reasonDetails, String phoneSessionId, String appSessionId) {
        ...
        try {
            UsageRollupDatabase.getInstance(this._context).collectAppSessionEvent(SessionEvent.Type.start, eventTimestamp, packageName, null);
        } catch (Exception e) {
    ...
    public boolean shouldFilterFromUsage(String packageName) {
        ...
        if (filterPattern == null || !filterPattern.matcher(packageName).matches()) {
            if (isFilterSystemEnabled()) {
                try {
                    long appFlags = this._context.getPackageManager().getPackageInfo(packageName, 128).applicationInfo.flags;
                    if ((appFlags & 1) == 1) {
                        return true;
                    }
                } catch (Exception e) {
                    Logger.w(Area.USAGE.value(), String.format(Locale.US, "UsageManager: Failed to get app flags [packageName:'%1$s' error:'%2$s']", packageName, e.getClass().getName()));
                }
            }
            return false;
        }
    ...
    public boolean shouldFilterTypeFromUsage(ReportUsageData.UsageType usageType) {
        ...
        synchronized (typeFilterLock) {
            ...
            if (typeFilterCache == null || !typeFilterCache.equals(typeFilter)) {
                ...
                if (typeFilter != null) {
                    String[] types = typeFilter.trim().split("\\|");
                    for (String type : types) {
                        if (type != null) {
                            try {
                                ReportUsageData.UsageType tempUsageType = ReportUsageData.UsageType.valueOf(type.trim());
                                typeFilterSetCache.add(tempUsageType);
                            } catch (IllegalArgumentException e) {
                                Logger.w(Area.USAGE.value(), String.format("Illegal value [%s] for usage type filter", type), e);
        ...
        Logger.d(Area.USAGE.value(), String.format(Locale.US, "UsageManager shouldFilterTypeFromUsage returning %s for %s", Boolean.toString(result), usageType.name()));
        return result;
    }
    ...
    public void closeAllOpenAppSessions(SessionEvent.Reason reason, String reasonDetails, long eventTime) {
        ...
        if (openSessions.size() > 0) {
            Logger.v(Area.USAGE.value(), String.format(Locale.US, "UsageManager: stopPhoneSession() Closing %1$d open app session(s)", Integer.valueOf(openSessions.size())));
