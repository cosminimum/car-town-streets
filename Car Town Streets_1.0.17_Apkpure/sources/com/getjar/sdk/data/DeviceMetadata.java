...
public class DeviceMetadata {
    ...
    private String findDeviceId(Context context) {
        ...
        if (!StringUtility.isNullOrEmpty(overrideDeviceID)) {
            Logger.v(Area.AUTH.value(), String.format(Locale.US, "[*** OVERRIDE ***] Override value being used: 'identity.device.id' = '%1$s'", overrideDeviceID));
    ...
    private String findSerialNo(Context context, int sdkLevel) {
        ...
        String overrideSerialNumber2 = OverridesUtility.getValueFakeID("identity.serial.number");
        if (!StringUtility.isNullOrEmpty(overrideSerialNumber2)) {
            Logger.v(Area.AUTH.value(), String.format(Locale.US, "[*** OVERRIDE ***] Override value being used: 'identity.serial.number' = '%1$s'", overrideSerialNumber2));
            return overrideSerialNumber2;
        }
        try {
            if (sdkLevel >= 9) {
                Field field = Build.class.getDeclaredField("SERIAL");
                overrideSerialNumber = (String) field.get(null);
            } else {
                Class<?> c = Class.forName("android.os.SystemProperties");
                Method get = c.getMethod("get", String.class, String.class);
                String serialNumber = (String) get.invoke(c, "ro.serialno", null);
                overrideSerialNumber = serialNumber;
            }
    ...
    private int findSdkInt(Context context) {
        try {
            Field sdkLevelField = Build.VERSION.class.getDeclaredField("SDK_INT");
            int sdkLevel = sdkLevelField.getInt(null);
            return sdkLevel;
        } catch (Exception e) {
            try {
                int sdkLevel2 = Integer.parseInt(Build.VERSION.SDK);
                return sdkLevel2;
            } catch (Exception e2) {
                return 3;
            }
    ...
    private String findHardware(Context context) {
        try {
            Field field = Build.class.getDeclaredField("HARDWARE");
            String hardware = (String) field.get(null);
            return hardware;
        } catch (Exception e) {
            return "";
        }
    ...
    private String findManufacturer(Context context) {
        try {
            ...
            String manufacturer = (String) field.get(null);
            return manufacturer;
        } catch (Exception e) {
            return "";
        }
