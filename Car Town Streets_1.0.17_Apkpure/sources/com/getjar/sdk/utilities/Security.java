...
public class Security {
    ...
    public static ArrayList<GooglePurchaseResponse> getPurchaseDetails(String signedData, String signature, Context context) {
        if (signedData == null) {
            ...
            try {
                ...
                Logger.w(Area.BUY_GOLD.value(), String.format(Locale.US, "JSONData: %1$s", jObject.toString()));
                ...
                for (int i = 0; i < numTransactions; i++) {
                    try {
                        ...
                        Constants.PurchaseState purchaseState = Constants.PurchaseState.valueOf(response);
                        ...
                        if (purchaseState == Constants.PurchaseState.PURCHASED) {
                            GooglePurchaseResponse resp = new GooglePurchaseResponse(notifyId, orderId, productId, (short) purchaseState.ordinal(), developerPayload, signedData, signature);
                            purchases.add(resp);
                        }
    ...
    public static PublicKey generatePublicKey(byte[] decodedKey) {
        try {
            ...
            Logger.e(Area.AUTH.value(), "Invalid key specification.");
    ...
    private static byte[] sign(PublicKey publicKey, String data) {
        ...
        try {
            MessageDigest sha1 = MessageDigest.getInstance(Constants.SIGNATURE_DIGEST);
            byte[] dataToSign = data.getBytes("UTF-8");
            byte[] hash = sha1.digest(dataToSign);
            Cipher cipher = Cipher.getInstance(Constants.SIGNATURE_CIPHER);
            cipher.init(1, publicKey);
            byte[] result = cipher.doFinal(hash);
            return result;
    ...
    public static boolean verifySignature(PublicKey publicKey, String signedData, String signature) {
        ...
        if (StringUtility.isNullOrEmpty(signedData)) {
        ...
        try {
            MessageDigest sha1 = MessageDigest.getInstance(Constants.SIGNATURE_DIGEST);
            byte[] hash = sha1.digest(signedData.getBytes("UTF-8"));
            Cipher cipher = Cipher.getInstance(Constants.SIGNATURE_CIPHER);
            ...
            if (hash.length == decryptedHash.length) {
                for (int i = 0; i < hash.length && hash[i] == decryptedHash[i]; i++) {
                    if (i == hash.length - 1) {
                        result = true;
                    }
                }
