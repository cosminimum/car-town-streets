...
public class GetJarJavaScriptInterface extends JavaScriptAPI {
    ...
    public GetJarJavaScriptInterface(CommContext gContext, GetJarWebViewSubActivity parentActivity, ResultReceiver receiver, List<GetJarWebViewSubActivity.ProductWithClientTransactionID> theOffers) {
        ...
        if (theOffers == null || theOffers.size() <= 0) {
            ...
            if (offer != null) {
                ...
                try {
                    ...
                    if (trackingMetadataMap.size() > 0) {
                        Logger.v(Area.JS_API.value() | Area.PURCHASE.value() | Area.TRANSACTION.value() | Area.AUTH.value(), String.format(Locale.US, "JavaScriptAPI: Making a purchase with client transaction ID: '%1$s' [thread:%2$d]", offer.getClientTransactionId(), Long.valueOf(Thread.currentThread().getId())));
                            ...
                            public void run() {
                                ...
                                try {
                                    try {
                                        ...
                                        if (!callbackMade) {
                                            callback.serviceRequestFailed(null, null, "unknown", GetJarJavaScriptInterface.this._commContext);
                                        }
                                    } catch (Exception e) {
    ...
    public void reloadViewNoSafety(boolean reauth) {
        Logger.d(Area.JS_API.value() | Area.UI.value(), String.format(Locale.US, "JavaScriptAPI: reloadViewNoSafety(%1$s)", Boolean.toString(reauth)));
        reloadViewInternal(reauth, false);
    }

    @JavascriptInterface
    public void reloadView(boolean reauth) {
        Logger.d(Area.JS_API.value() | Area.UI.value(), String.format(Locale.US, "JavaScriptAPI: reloadView(%1$s)", Boolean.toString(reauth)));
    ...
    public void launchGetJarRewardsApp() {
        if (!this._isExist) {
            launch(Constants.GREENJAR_PACKAGE, null);
        }
    }
    ...
    public String getProductThumbnailString(String productId, String boundWidth, String boundHeight) {
        ...
        try {
            ...
            if (resourceId != null && resourceId.intValue() > 0) {
                ...
                image = Utility.getImageDataFromResource(this._parentActivity.getParentActivity(), resourceId, Integer.parseInt(boundWidth), Integer.parseInt(boundHeight));
                Logger.d(Area.JS_API.value() | Area.PURCHASE.value(), "JavaScriptAPI: getProductThumbnailString() resource found");
            } else {
                Logger.d(Area.JS_API.value() | Area.PURCHASE.value(), String.format(Locale.US, "JavaScriptAPI: getProductThumbnailString() no resource ID found for product '%1$s', returning null", productId));
            }
        } catch (Exception e) {
            ...
            public void run() {
                try {
                    ...
                    GetJarJavaScriptInterface.this._parentActivity.getParentActivity().runOnUiThread(new Runnable() { // from class: com.getjar.sdk.rewards.GetJarJavaScriptInterface.2.1
                        ...
                        public void run() {
                            try {
                                FrameLayout mFrame = (FrameLayout) GetJarJavaScriptInterface.this._parentActivity.getParentActivity().findViewById(16908290);
                                WebView mWebView = (WebView) mFrame.getChildAt(0);
                                mWebView.loadUrl(String.format(Locale.US, "javascript:GJ.receivedGoldOffers(%s)", offers));
                            } catch (Exception e) {
    ...
    public String getAuthToken() {
        ...
        try {
            ...
            Logger.d(Area.JS_API.value() | Area.AUTH.value(), String.format(Locale.US, "JavaScriptAPI: getAuthToken() returning: '%1$s'", result));
            return result;
        } catch (Exception e) {
    ...
    public String getDisplayLanguageTag() {
        ...
        try {
            ...
            Logger.d(Area.JS_API.value() | Area.UI.value() | Area.LOCALIZATION.value(), String.format(Locale.US, "JavaScriptAPI: getDisplayLanguageTag() returning: '%1$s'", result));
            return result;
        } catch (Exception e) {
    ...
    public String getRecentlyUsedApps() {
        ...
        try {
            if (UsageManager.getInstance(this._commContext.getApplicationContext()).isRequestSendEnabled()) {
                ...
                Logger.d(Area.JS_API.value() | Area.USAGE.value(), String.format(Locale.US, "JavaScriptAPI: getRecentlyUsedApps() %d recent apps", Integer.valueOf(recentPackageNames.size())));
                for (String packageName : recentPackageNames) {
                    appArray.put(packageName);
                }
            }
            Logger.d(Area.JS_API.value() | Area.USAGE.value(), String.format(Locale.US, "JavaScriptAPI: getRecentlyUsedApps() returning:\r\n%1$s", appArray.toString(4)));
        } catch (Exception e) {
        ...
        try {
            ...
            if (report != null) {
                ...
                if (report.getRecords().size() > 0) {
                    Logger.v(Area.JS_API.value() | Area.USAGE.value(), String.format(Locale.US, "JavaScriptAPI: getUsageSessionRecords() reporting on %1$d aggregated usage session records", Integer.valueOf(report.getRecords().size())));
    ...
    public void log(String message, String level) {
        ...
        String message2 = String.format("JavaScript: %s", message);
    ...
    public void configureWebViewOnUIThread(Boolean builtInZoom, Boolean supportZoom, String zoomDensity) {
        if (Utility.isCurrentThreadTheUIThread()) {
            ...
            if (zoomDensity != null) {
                Object zoomDensityEnum = getZoomDensity(zoomDensity);
                WebSettingsEx.setDefaultZoom(webSettings, zoomDensityEnum);
            }
            if (builtInZoom != null) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child != null) {
                if (WebView.class.isAssignableFrom(child.getClass())) {
                    WebView webView2 = (WebView) child;
                    return webView2;
                } else if (ViewGroup.class.isAssignableFrom(child.getClass()) && (webView = findWebViewInUILayout((ViewGroup) child)) != null) {
                    return webView;
                }
            }
        }
    ...
    private Object getZoomDensity(String zoomDensityString) {
        ...
        if (zoomDensityString.equals("FAR")) {
    ...
    private void _requestInstall(String thePackageName, String theFriendlyName, String theDownloadUrl, String theAppMetadata, String theTrackingMetadata, Constants.RequestInstallType theRequestType) {
        ...
        if (StringUtility.isNullOrEmpty(theFriendlyName)) {
            throw new IllegalArgumentException("'theFriendlyName' cannot be NULL or empty");
        }
        if (StringUtility.isNullOrEmpty(theDownloadUrl)) {
        ...
        if (StringUtility.isNullOrEmpty(theTrackingMetadata)) {
        ...
        try {
            Logger.d(Area.JS_API.value() | Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "JavaScriptAPI: _requestInstall(packageName:%1$s, friendlyName:%2$s, downloadUrl:%3$s, requestType:%4$s)", thePackageName, theFriendlyName, theDownloadUrl, theRequestType.name()));
            Logger.v(Area.JS_API.value() | Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "JavaScriptAPI: _requestInstall() applicationMetadata: %1$s", theAppMetadata));
            Logger.v(Area.JS_API.value() | Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "JavaScriptAPI: _requestInstall() trackingMetadata: %1$s", theTrackingMetadata));
            ...
            Logger.d(Area.JS_API.value() | Area.EARN.value() | Area.TRANSACTION.value(), String.format(Locale.US, "JavaScriptAPI: Pushing download URL to the Android OS [downloadURl: %1$s]", theDownloadUrl));
        ...
        if (!this._isExist) {
            ...
            if (reauth) {
                ...
                if (doSafety && lastAttemptDelta < TapjoyConstants.THROTTLE_GET_TAP_POINTS_INTERVAL) {
                    Logger.w(Area.JS_API.value() | Area.UI.value(), String.format(Locale.US, "JavaScriptAPI: A re-authorization/reload attempt was made %1$d milliseconds ago, skipping this attempt request", Long.valueOf(lastAttemptDelta)));
            ...
            try {
                try {
                    ...
                    if (!substate.equals(Constants.RequestInstallSubState.FUNDS_INSUFFICIENT_FAILURE.name())) {
                        ...
                        GetJarJavaScriptInterface.this._successBundle = b;
        ...
        public void serviceRequestFailed(Result result, Exception cause, String requestId, CommContext commContext) {
            try {
                try {
                    ...
                    b.putLong("price", this._offer.getProduct().getAmount());
                    b.putString("name", this._offer.getProduct().getProductName());
                    String substate = Utility.getResponseSubstate(cause, Constants.RequestInstallSubState.NONE.toString());
                    b.putString(Constants.RequestInstallSubState.KEY(), substate);
                    long value = Area.JS_API.value() | Area.PURCHASE.value() | Area.TRANSACTION.value();
                    Locale locale = Locale.US;
                    ...
                    objArr[0] = requestId;
                    objArr[1] = commContext == null ? "" : commContext.getCommContextId();
                    objArr[2] = cause == null ? "" : cause.getMessage();
                    ...
                    GetJarJavaScriptInterface.this._failBundle = b;
                    Message msg = new Message();
                    msg.what = 1;
                    GetJarJavaScriptInterface.this._purchaseResultHandler.sendMessage(msg);
                } catch (Exception e) {
