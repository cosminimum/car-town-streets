...
public final class GetJarPage {
    ...
    public GetJarPage(GetJarContext getJarContext) {
        Logger.d(Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage");
    ...
    public void setProducts(Collection<Product> products) {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProducts()");
    ...
    public void setProduct(Product product) {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProduct()");
        if (product == null) {
    ...
    public void setLicensableProduct(String theProductId, String theProductName, String theProductDescription, long theAmount, License.LicenseScope licenseScope) {
        Logger.d(Area.PURCHASE.value() | Area.LICENSING.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProduct()");
        Product licensableProduct = new LicensableProduct(theProductId, theProductName, theProductDescription, theAmount, licenseScope);
        ArrayList<Product> newProducts = new ArrayList<>(1);
        newProducts.add(licensableProduct);
        this.mProducts = Collections.unmodifiableCollection(newProducts);
    }
    ...
    public void setLicensableProduct(String theProductId, String theProductName, String theProductDescription, long theAmount, int imageResourceId, License.LicenseScope licenseScope) {
        Logger.d(Area.PURCHASE.value() | Area.LICENSING.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProduct()");
        Product licensableProduct = new LicensableProduct(theProductId, theProductName, theProductDescription, theAmount, imageResourceId, licenseScope);
        ArrayList<Product> newProducts = new ArrayList<>(1);
        newProducts.add(licensableProduct);
        this.mProducts = Collections.unmodifiableCollection(newProducts);
    }
    ...
    public void setConsumableProduct(String theProductId, String theProductName, String theProductDescription, long theAmount) {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProduct()");
        Product consumableProduct = new ConsumableProduct(theProductId, theProductName, theProductDescription, theAmount);
        ArrayList<Product> newProducts = new ArrayList<>(1);
        newProducts.add(consumableProduct);
        this.mProducts = Collections.unmodifiableCollection(newProducts);
    }
    ...
    public void setConsumableProduct(String theProductId, String theProductName, String theProductDescription, long theAmount, int imageResourceId) {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.setProduct()");
        Product consumableProduct = new ConsumableProduct(theProductId, theProductName, theProductDescription, theAmount, imageResourceId);
        ArrayList<Product> newProducts = new ArrayList<>(1);
        newProducts.add(consumableProduct);
        this.mProducts = Collections.unmodifiableCollection(newProducts);
    }
    ...
    public void setGoogleInAppBillingKey(String publicKey) {
        ...
        SharedPreferences prefs = this.mGjContext.getAndroidContext().getSharedPreferences("GetJarClientPrefs", 0);
    ...
    public Collection<Product> getProducts() {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "GetJarPage.getProducts()");
        return this.mProducts;
    }

    public void showPage() {
        Logger.d(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), "showPage() -- deviceObject Id:" + Utility.getDeviceObjectId(this.mGjContext.getAndroidContext()));
        ...
        for (Product product : this.mProducts) {
            ...
            if (product instanceof LicensableProduct) {
                licenseScope = String.format(Locale.US, " LicenseScope:%1$s", ((LicensableProduct) product).getLicenseScope().name());
            }
            Logger.v(Area.PURCHASE.value() | Area.UI.value() | Area.DEVELOPER_API.value(), String.format(Locale.US, "GetJarPage: showPage() PRODUCT [ProductName:'%1$s' ProductDescription:'%2$s' ProductId:'%3$s' ProductAmount:%4$d ProductImageResourceId:%5$d%6$s]", product.mProductName, product.mProductDescription, product.mProductId, Long.valueOf(product.mProductAmount), product.mProductImageResourceId, licenseScope));
        }
