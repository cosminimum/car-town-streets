...
public class Result implements Serializable {
    ...
    private String getResponseBodyValueAsString(String key) {
        try {
            ...
            Logger.e(Area.COMM.value(), String.format(Locale.US, "Unable to get a string value for '%1$s'", key), e);
        }
        return null;
    }
    ...
    private String getResponseBodyValueAsRawString(String key) {
        ...
        try {
            if (this._responseJson != null && this._responseJson.has("return") && this._responseJson.getJSONObject("return").has(key)) {
                int start = this._responseBody.indexOf(String.format(Locale.US, "\"%1$s\"", key));
                ...
                switch (startChar) {
                    ...
                    case '{':
                        ...
                        break;
                    default:
                        throw new IllegalStateException(String.format(Locale.US, "Unrecognized start character '%1$c'", Character.valueOf(startChar)));
                }
                int setCount = this._responseJson.getJSONObject("return").getString(key).split(String.format("\\%1$c", Character.valueOf(endChar))).length - 1;
                int end = start;
                int setFound = 0;
            ...
            Logger.e(Area.COMM.value(), String.format(Locale.US, "Unable to get a raw string value for '%1$s'", key), e);
        }
        return null;
    }
    ...
    public boolean checkForUnauthorizedAndOKToReAuth(CommContext commContext) throws JSONException {
        ...
        boolean isUnauthorizedAndOKToReAuth = isUnauthorized && !isNotOKToReAuth;
        if (isUnauthorized) {
    ...
    public boolean checkForBlacklistedOrUnsupported(CommContext commContext) throws JSONException {
        ...
        objArr[0] = Boolean.toString(blacklisted || unsupported).toUpperCase();
        ...
        return blacklisted || unsupported;
    }
    ...
    public boolean checkForUnsupported() throws JSONException {
        if (checkForCallerUnauthorized() && this._responseJson.getJSONObject(GCMConstants.EXTRA_ERROR).has("subcode")) {
            String subcode = this._responseJson.getJSONObject(GCMConstants.EXTRA_ERROR).getString("subcode");
            if (subcode.equalsIgnoreCase(OnErrorConstants.SUBCODE_AUTH_UNSUPPORTED_DEVICE)) {
                return true;
            }
        }
        return false;
