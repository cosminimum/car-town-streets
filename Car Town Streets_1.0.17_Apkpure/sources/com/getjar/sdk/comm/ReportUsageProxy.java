...
public class ReportUsageProxy extends ServiceProxyBase {
    ...
    public Operation reportApplicationUsage(CommContext commContext, List<ReportUsageData> appUsageList) {
        ...
        Logger.d(Area.USAGE.value() | Area.COMM.value(), String.format(Locale.US, "Filtered usage records to report from %1$d to %2$d", Integer.valueOf(appUsageList.size()), Integer.valueOf(filteredAppUsageList.size())));
        ...
        while (appUsageItr.hasNext()) {
            try {
                postDataBuffer.append(appUsageDataToJson(appUsageItr.next()));
        ...
        postData.put("app_usage_data", postDataBuffer.toString());
        AuthManager.initialize(commContext.getApplicationContext());
        AuthManager.getInstance().waitOnAuth();
        try {
            String url = String.format(Locale.US, _URL_TEMPLATE_APPLICATION_REPORT_USAGE, GetJarConfig.getInstance(commContext, true).getDirectiveValue(GetJarConfig.KEY_REPORT_USAGE_ENDPOINT), URLEncoder.encode(AuthManager.getInstance().getUserDeviceId(), "UTF-8"));
            return makeAsyncPOSTRequestForJson("reportApplicationUsage", Operation.Priority.LOW, commContext, url, postData, null, null, true, true, false);
    ...
    private List<ReportUsageData> filterAppUsageDataList(Context context, List<ReportUsageData> appUsageList) {
        ...
        for (ReportUsageData appData : appUsageList) {
            if (StringUtility.isNullOrEmpty(appData.getPackageName()) || !UsageManager.getInstance(context).shouldFilterFromUsage(appData.getPackageName())) {
                if (!UsageManager.getInstance(context).shouldFilterTypeFromUsage(appData.getUsageType())) {
