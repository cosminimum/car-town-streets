...
public class LicenseServiceProxy extends ServiceProxyBase {
    ...
    public Operation acquireUnmanagedProductLicense(CommContext commContext, String itemId, License.LicenseScope licenseScope, HashMap<String, String> trackingMetadata) {
        ...
        try {
            ...
            String signature = generateSignatureForAcquire(AuthManager.getInstance().getUserDeviceId(), itemId, licenseScope, nonce, authToken, commContext.getAppEncryptionKeyIndex(), commContext.getAppEncryptionPublicKey());
            ...
            try {
                String url = String.format(Locale.US, _URL_TEMPLATE_ACQUIRE_LICENSE, GetJarConfig.getInstance(commContext, true).getDirectiveValue(GetJarConfig.KEY_LICENSE_SERVICE_ENDPOINT), URLEncoder.encode(signature, "UTF-8"));
                return makeAsyncPOSTRequestForJson("acquireUnmanagedProductLicense", Operation.Priority.HIGH, commContext, url, postData, null, null, true, true, true);
    ...
    public Operation getUnmanagedProductLicenses(CommContext commContext, License.LicenseScope licenseScope, LicenseInternal.ExternalLicenseState licenseState, String continuationToken, int limit, String eTag) {
        ...
        String nonce = generateNonce();
        String authToken = AuthManager.getInstance().getAuthToken();
        String signature = generateSignatureForGet(AuthManager.getInstance().getUserDeviceId(), nonce, authToken, commContext.getAppEncryptionKeyIndex(), commContext.getAppEncryptionPublicKey());
        commContext.putSignature(nonce, signature.substring(4));
        try {
            Locale locale = Locale.US;
            String str = _URL_TEMPLATE_GET_LICENSES;
            ...
            objArr[3] = URLEncoder.encode(licenseScope != null ? licenseScope.toString() : "", "UTF-8");
            objArr[4] = URLEncoder.encode(licenseState != null ? licenseState.toString() : "", "UTF-8");
            ...
            return makeAsyncGETRequestForJson("getUnmanagedProductLicenses", Operation.Priority.HIGH, commContext, url, requestHeaders, null, true, true, true);
        } catch (UnsupportedEncodingException e) {
            throw new CommunicationException(e);
        }
    }
    ...
    public static String generateSignDataForAcquire(String userDeviceId, String itemId, License.LicenseScope licenseScope, String nonce, String authHeader) {
        ...
        if (StringUtility.isNullOrEmpty(authHeader)) {
            throw new IllegalArgumentException("authHeader cannot be null");
        }
        String data = String.format(Locale.US, "%s%s%s%s%s", userDeviceId, itemId, licenseScope.name(), nonce, authHeader);
        return data;
    }
    ...
    public static String generateSignDataForGet(String userDeviceId, String nonce, String authHeader) {
        ...
        String data = String.format(Locale.US, "%s%s%s", userDeviceId, nonce, authHeader);
        return data;
    }
    ...
    public static String generateSignatureForAcquire(String userDeviceId, String itemId, License.LicenseScope licenseScope, String nonce, String authHeader, String keyIndex, PublicKey publicKey) {
        ...
        String data = generateSignDataForAcquire(userDeviceId, itemId, licenseScope, nonce, authHeader);
        String signature = generateSignature(keyIndex, publicKey, data);
        return signature;
    }
    ...
    public static String generateSignatureForGet(String userDeviceId, String nonce, String authHeader, String keyIndex, PublicKey publicKey) {
        ...
        String data = generateSignDataForGet(userDeviceId, nonce, authHeader);
        String signature = generateSignature(keyIndex, publicKey, data);
        return signature;
    }
    ...
    public static String generateSignature(String keyIndex, PublicKey publicKey, String data) {
        ...
        String signature = String.format(Locale.US, "%s%s", keyIndex, Security.generateSignature(publicKey, data));
        return signature;
    }
}
