...
public class LocalizationServiceProxy extends ServiceProxyBase {
    ...
    public Operation getRecommendedPrices(CommContext commContext, Collection<Pricing> prices) {
        ...
        String url = String.format(Locale.US, _URL_TEMPLATE_RECOMMENDED_PRICES, GetJarConfig.getInstance(commContext, true).getDirectiveValue(GetJarConfig.KEY_LOCALIZATION_SERVICE_ENDPOINT));
        ...
        try {
            ...
            return makeAsyncPOSTRequestForJson("getRecommendedPrices", Operation.Priority.MEDIUM, commContext, url, postData, null, null, true, true, true);
    ...
    public Operation getGoldOffers(CommContext commContext) {
        ...
        AuthManager.initialize(commContext.getApplicationContext());
        AuthManager.getInstance().waitOnAuth();
        Locale currentLocale = commContext.getApplicationContext().getResources().getConfiguration().locale;
        String isoCountryCode = currentLocale.getCountry();
        try {
            String url = String.format(Locale.US, _URL_TEMPLATE_GOLD_BUCKETS, GetJarConfig.getInstance(commContext, true).getDirectiveValue(GetJarConfig.KEY_LOCALIZATION_SERVICE_ENDPOINT), URLEncoder.encode(isoCountryCode, "UTF-8"));
            return makeAsyncGETRequestForJson("getGoldOffers", Operation.Priority.MEDIUM, commContext, url, null, null, true, false, true);
